...
internal static class CsvSchemaGenerator
{
    public static CsvSchema? Generate(Type type)
    {
        ...
        if (schemaAttribute is null) return null;

        var columns = schemaAttribute.RowType
            .GetProperties(BindingFlags.Public | BindingFlags.Instance)
            .Select(BuildColumnSchema)
            .ToArray();

        return new CsvSchema { Columns = columns };
    }
    ...
    {
        return property.GetCustomAttribute<NameAttribute>()?.Names ?? [];
    }
}
