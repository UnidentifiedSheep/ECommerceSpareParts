...
public class DocumentRepository<TEntity, TKey>(IDocumentSession session) : IDocumentRepository<TEntity, TKey>
	where TKey : notnull where TEntity : Entity<TEntity, TKey>
{
	public ValueTask<TEntity?> GetById(TKey id, CancellationToken ct = default) =>
		new(session.LoadAsync<TEntity>(id, ct));
	...
	public async Task<List<TEntity>> ListAsync(
		...
		CancellationToken ct = default)
	{
		var result = await ApplyCriteria(session.Query<TEntity>(), criteria).ToListAsync(ct);
	...
	public void Add(TEntity entity) => Insert(entity);

	public void Add(IEnumerable<TEntity> entities) => Insert(entities);
	...
	private static IQueryable<TEntity> ApplyCriteria(IQueryable<TEntity> query, Criteria<TEntity>? criteria)
	{
		ValidateCriteria(criteria);
		if (criteria is null)
			return query;
