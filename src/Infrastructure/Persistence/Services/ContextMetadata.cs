...
public class ContextMetadata<TContext>(TContext context) : IContextMetadata where TContext : DbContext
{
	...
	public string GetColumnName<TEntity>(LambdaExpression keySelector)
	{
		...
		return ColumnNameCache.GetOrAdd(
			...
			_ =>
			{
				...
				var property = efEntity.FindProperty(propertyName) ?? throw new InvalidOperationException(
	...
	{
		var entityType = typeof(TEntity);
		return TableNameCache.GetOrAdd(
			entityType,
			_ =>
			{
				return context.Model.FindEntityType(entityType)?.GetTableName() ??
					throw new ArgumentNullException(
						$"Сущность {entityType.Name} не относится к сущностям бд.");
			});
	}
	...
	{
		var entityType = typeof(TEntity);
		return SchemaNameCache.GetOrAdd(
			entityType,
			_ =>
			{
