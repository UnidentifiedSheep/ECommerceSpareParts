...
public class QueryableExtensions(IContextMetadata contextMetadata) : IQueryableExtensions
{
	...
	public IQueryable<T> ForUpdate<T>(
		IQueryable<T> query,
		bool forUpdate = true,
		bool skipLocked = false) where T : class
	{
		...
		query = query.TagWith("ForUpdate").TagWith($"ForUpdateOf:{fullQualifiedTableName}");

		return skipLocked ? query.TagWith("SkipLocked") : query;
	}
