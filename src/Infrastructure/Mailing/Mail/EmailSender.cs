...
public class EmailSender(IOptions<MailOptions> options) : IEmailSender
{
	public async Task SendAsync(IEmailMessage message, CancellationToken token = default) =>
		await SendBatchAsync([message], token);

	public async Task SendBatchAsync(IEnumerable<IEmailMessage> messages, CancellationToken token = default)
	{
	...
	private static MimeMessage BuildMessage(IEmailMessage message, MailOptions opt)
	{
	...
	private sealed record SendMessageState(SmtpClient Client, MimeMessage Message, MailOptions Options);
}
