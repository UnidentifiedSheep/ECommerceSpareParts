...
public sealed record GetPriceItem
{
    ...
    public DateTimeOffset? ExpectedDeliveryDate =>
        ParseDate(ExpectedDeliveryDateRaw);
    ...
    [JsonIgnore]
    public DateTimeOffset? GuaranteedDeliveryDate =>
        ParseDate(GuaranteedDeliveryDateRaw);

    [JsonPropertyName("PriceLastUpdateDate")]
    public string? PriceUpdatedAtRaw { get; init; }

    [JsonIgnore]
    public DateTimeOffset? PriceUpdatedAt =>
        ParseDate(PriceUpdatedAtRaw);
