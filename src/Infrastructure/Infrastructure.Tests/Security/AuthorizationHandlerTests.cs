...
public class AuthorizationHandlerTests
{
	...
	public async Task PermissionHandler_WhenAnyPermissionMatches_Succeeds()
	{
		...
		var context = await Authorize(new PermissionAuthorizationHandler(userContext), requirement);
	...
	{
		var userContext = CreateUser(false, ["purchase_create"]);
		var requirement = new PermissionRequirement(["PURCHASE_CREATE"], AuthorizationMatch.Any);
	...
	public async Task RoleHandler_NormalizesRolesAndSupportsAllMatch()
	{
		...
		var requirement = new RoleRequirement(["ADMIN", " Operator "], AuthorizationMatch.All);

		var context = await Authorize(new RoleAuthorizationHandler(userContext), requirement);
