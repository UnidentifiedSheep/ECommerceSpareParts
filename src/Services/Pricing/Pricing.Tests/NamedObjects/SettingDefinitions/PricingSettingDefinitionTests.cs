...
public class PricingSettingDefinitionTests
{
	...
	{
		_settingsService
			.Setup(x => x.SetSetting(It.IsAny<PricingSetting>(), It.IsAny<CancellationToken>()))
			.Callback<PricingSetting, CancellationToken>((setting, _) => _savedSettings.Add(setting))
			.Returns(Task.CompletedTask);

		_definition = new PricingSettingDefinition(_settingsService.Object, _publisher);
	}
	...
	public async Task UpdateSettingAsync_WithValidInput_PublishesMarkupRefreshRequestedEvent()
	{
		...
		_publisher.PublishedMessagesOfType<MarkupRangesRefreshRequestedEvent>().Should().ContainSingle();
	}

	[Fact]
	public async Task UpdateSettingAsync_WithInvalidInput_DoesNotSaveOrPublishEvent()
	{
		var invalidInput = ValidInput() with
		{
			DefaultMarkup = -0.1m
		};
		...
		_publisher.PublishedMessagesOfType<MarkupRangesRefreshRequestedEvent>().Should().BeEmpty();
	}
