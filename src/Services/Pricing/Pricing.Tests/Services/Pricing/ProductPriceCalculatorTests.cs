...
public class ProductPriceCalculatorTests
{
	...
	public ProductPriceCalculatorTests()
	{
		...
		_marketInfoFactory
			.Setup(x => x.CreateFromSupplierPrices(It.IsAny<IReadOnlyCollection<CalculatedPriceCandidate>>()))
	...
	public async Task CalculateAsync_ReturnsVersionsUsedForCalculation()
	{
		...
		var result = await _calculator.CalculateAsync([supplier, internalCandidate], CancellationToken.None);
