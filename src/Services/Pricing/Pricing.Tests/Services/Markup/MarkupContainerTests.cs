...
public class MarkupContainerTests
{
	...
	public void GetForDefaultOrNull_WithMatchingRange_ReturnsMarkup(int valueInCents, int expectedPercent)
	{
	...
	public void Initialize_WithLargeRange_DoesNotDependOnRangeCardinality()
	{
		...
		container.GetForDefaultOrNull(4_999_999.99m).Should().Be(new MarkupModel(0.2m));
	}
	...
	public void Initialize_WithRangesInDifferentOrder_GeneratesSameVersion()
	{
		var first = CreateContainer(
			...
			new Dictionary<int, IEnumerable<MarkupRange>>
			{
				[3] = CurrencyRanges(3), [2] = CurrencyRanges(2)
			});
		var second = CreateContainer(
			DefaultRanges().Reverse(),
			new Dictionary<int, IEnumerable<MarkupRange>>
			{
				[2] = CurrencyRanges(2).Reverse(), [3] = CurrencyRanges(3).Reverse()
			});
	...
	private static MarkupRange Range(
		decimal start,
		decimal end,
		decimal markup) => MarkupRange.Create(
		start,
		end,
		markup);
}
