...
public class GetPriceAppliersTests(CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
	...
	public async Task ConfigurationSnapshot_IncludesRegistryOnlyLocalAppliers()
	{
		...
		var markup = configuration.Appliers.Single(x => x.SystemName == nameof(MarkupApplier));
		markup.IsDynamic.Should().BeFalse();
		markup.States.Should().HaveCount(2);
		markup.States.Should().OnlyContain(x => x.Enabled && x.Order == 0);
		markup
			.States
			.Select(x => x.Usage)
			.Should()
			.BeEquivalentTo([PriceOfferSourceType.Supplier, PriceOfferSourceType.OurWarehouse]);
	...
	{
		var result = await Mediator.Send(new GetPriceApplierQuery(nameof(PriceRoundingApplier)));
	...
	public async Task BySystemName_ReturnsPersistedDynamicApplier()
	{
		...
		var result = await Mediator.Send(new GetPriceApplierQuery(applier.SystemName));
		...
		result
			...
			.States
			.Should()
			.ContainSingle(x => x.Usage == PriceOfferSourceType.Supplier && x.Order == 50 && !x.Enabled);
	}
	...
	public async Task BySystemName_ReturnsPersistedStateForLocalApplier()
	{
		...
		var result = await Mediator.Send(new GetPriceApplierQuery(nameof(MarkupApplier)));
		...
		result
			...
			.States
			.Should()
			.Contain(x => x.Usage == PriceOfferSourceType.Supplier && x.Order == 0 && !x.Enabled);
		result
			.Applier
			.States
			.Should()
			.Contain(x => x.Usage == PriceOfferSourceType.OurWarehouse && x.Order == 0 && !x.Enabled);
	}
	...
	public async Task BySystemName_MissingApplier_ThrowsNotFoundException()
	{
		...
		await Assert.ThrowsAsync<PriceApplierNotFoundException>(() => Mediator.Send(query));
	}
	...
	public async Task ForUsage_ReturnsLocalAndDynamicAppliersIncludingDisabled()
	{
		...
		var result = await Mediator.Send(new GetPriceAppliersQuery(PriceOfferSourceType.Supplier));

		result
			.Appliers
			.Should()
			.Contain(x =>
				x.SystemName == nameof(MarkupApplier) && x.Name != x.SystemName && !x.IsDynamic &&
				!x.States.Single().Enabled);
		result
			.Appliers
			.Should()
			.Contain(x =>
				x.SystemName == nameof(PriceRoundingApplier) && !x.IsDynamic && x.States.Single().Enabled);
		result
			.Appliers
			.Should()
			.Contain(x =>
				x.SystemName == enabledDynamic.SystemName && x.Name == enabledDynamic.Name && x.IsDynamic &&
				x.States.Single().Enabled);
		result
			.Appliers
			.Should()
			.Contain(x =>
				x.SystemName == disabledDynamic.SystemName && x.IsDynamic && !x.States.Single().Enabled);
		result.Appliers.Should().NotContain(x => x.SystemName == otherUsageDynamic.SystemName);
		result
			.Appliers
			.Should()
			.OnlyContain(x => x.States.Count == 1 && x.States[0].Usage == PriceOfferSourceType.Supplier);
	}
