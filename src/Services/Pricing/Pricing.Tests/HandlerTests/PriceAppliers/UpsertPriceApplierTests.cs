...
public class UpsertPriceApplierTests(CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
	...
	public async Task WithNewDynamicApplier_CreatesApplierAndStates()
	{
		...
		applier.States.Should().BeEquivalentTo(command.States, options => options.ExcludingMissingMembers());
	}
	...
	public async Task WithNewLocalApplier_UsesLocalOrderAndIgnoresDslLogic()
	{
		var command = new UpsertPriceApplierCommand(
			...
			null,
			"not-json",
			[State(PriceOfferSourceType.Supplier, 999), State(PriceOfferSourceType.OurWarehouse, null)]);
	...
	public async Task WithExistingLocalApplier_RemovesMissingStateAndDoesNotApplyIt()
	{
		...
		var warehouseAppliers = await service.GetPriceAppliersAsync(PriceOfferSourceType.OurWarehouse);
	...
	public async Task WithCachedAppliers_CreatingApplierInvalidatesCache()
	{
		...
		appliers.Should().Contain(x => x.SystemName == systemName && x.Name == "Created rule");
	...
	public async Task WithCachedAppliers_UpdatingApplierInvalidatesCache()
	{
		...
		var updatedConfiguration = await provider.GetConfigurationAsync();
		updatedConfiguration.Version.Should().NotBe(initialConfiguration.Version);
		var applier = updatedConfiguration.Appliers.Single(x => x.SystemName == existing.SystemName);
		applier.Name.Should().Be("Updated rule");
		JsonNode
			.DeepEquals(JsonNode.Parse(applier.DslLogic!), JsonNode.Parse("""{"var":"cost"}"""))
			.Should()
			.BeTrue();
	}
	...
	public async Task WithCachedAppliers_UpdatingLocalStateInvalidatesCache()
	{
		...
		updatedConfiguration.Version.Should().NotBe(initialConfiguration.Version);
		var applier = updatedConfiguration.Appliers.Single(x => x.SystemName == existing.SystemName);
		applier.States.Single(x => x.Usage == PriceOfferSourceType.Supplier).Enabled.Should().BeTrue();
	}
