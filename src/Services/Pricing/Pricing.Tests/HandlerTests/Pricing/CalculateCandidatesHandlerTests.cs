...
public class CalculateCandidatesHandlerTests(CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
	...
	public async Task Handle_WithCalculatedCandidate_PersistsAllConfigurationVersions()
	{
		...
		await handler.Handle(new CalculateCandidatesCommand(ProductId, StorageCode), CancellationToken.None);

		var option = await Context
			.Set<ProductPriceOption>()
			.AsNoTracking()
			.SingleAsync(x => x.PriceOfferId == offer.Id, CancellationToken);
