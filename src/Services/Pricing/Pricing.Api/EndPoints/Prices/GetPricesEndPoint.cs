...
public class GetPricesEndPoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        app.MapGet("/prices", async (
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                if (!detailed)
                    return await GetNormal(sender, articleIds, buyerId, currencyId, signer, cancellationToken);
    ...
    private async Task<IResult> GetDetailed(
        ISender sender,
    ...
    private async Task<IResult> GetNormal(
        ISender sender,
        IEnumerable<int> articleIds,
        Guid? buyerId,
        int currencyId,
