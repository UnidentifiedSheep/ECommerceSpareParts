...
public class PriceApplierEndPoints : ICarterModule
{
	...
	{
		var priceAppliers = app.MapGroup("/price-appliers").WithTags("Price appliers");

		priceAppliers
			.MapGet(
				"",
				async (
					ISender sender, [AsParameters] GetPriceAppliersRequest request,
			...
			.WithDescription(
				"Возвращает локальные и динамические правила для выбранного источника предложения")
		...
		priceAppliers
			.MapGet(
				...
				async (
					...
					CancellationToken cancellationToken) =>
				{
					var result = await sender.Send(new GetPriceApplierQuery(systemName), cancellationToken);

					return Results.Ok(new GetPriceApplierResponse(result.Applier));
				})
		...
		priceAppliers
			.MapPost(
				"",
				async (
					ISender sender, UpsertPriceApplierRequest request,
					CancellationToken cancellationToken) =>
				{
		...
		priceAppliers
			.MapDelete(
				...
				async (
					...
					CancellationToken cancellationToken) =>
				{
					await sender.Send(new DeletePriceApplierCommand(systemName), cancellationToken);
