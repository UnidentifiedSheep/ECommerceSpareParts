...
public class MarkupEndPoints : ICarterModule
{
	...
	{
		var markups = app.MapGroup("/markups").WithTags("Markups");

		markups
			.MapPost(
				"",
				async (
					ISender sender, UpsertMarkupGroupRequest request,
		...
		markups
			.MapGet(
				"",
				async (
					ISender sender, [AsParameters] PaginationQueryModel request,
					CancellationToken cancellationToken) =>
				{
