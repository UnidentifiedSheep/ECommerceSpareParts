        ...
        => new()
        {
            SystemName = systemName
                ...
                .EnsureNotNullOrWhiteSpace(
                    () => new InvalidOperationException("System name cannot be empty")),
            DslLogic = dslLogic
                .TrimSafe()
                .EnsureNotNullOrWhiteSpace(
                    () => new InvalidOperationException("DSL logic cannot be empty"))
                .EnsureValidJson(
                    () => new InvalidOperationException("DSL logic is not valid JSON"))
        };
