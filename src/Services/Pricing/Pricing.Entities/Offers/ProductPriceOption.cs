...
public class ProductPriceOption : AuditableEntity<ProductPriceOption, Guid>,
	ILinqEntity<ProductPriceOption, Guid>
{
	...
	public static Expression<Func<ProductPriceOption, bool>> GetEqualityExpression(Guid key) => x =>
		x.PriceOfferId == key;
	...
	public void SetScore(decimal score) => Score =
		score.EnsureNonNegative(() => new InvalidOperationException("Score cannot be negative"));

	public void SetPrice(decimal priceInBaseCurrency) => Price = priceInBaseCurrency.EnsureGreaterThan(
		0,
		() => new InvalidOperationException("Price cannot be negative"));

	public void SetMarkup(decimal markup) => Markup =
		markup.EnsureNonNegative(() => new InvalidOperationException("Markup cannot be negative"));
	...
	public void SetDeliveryTime(
		...
		int deliveryProbability)
	{
		deliveryProbability.EnsureNonNegative(() =>
			new InvalidOperationException("Delivery probability cannot be negative"));
		deliveryTime.EnsureAtLeast(
			TimeSpan.Zero,
			() => new InvalidOperationException("Delivery time cannot be negative"));
		guaranteedDeliveryTime.EnsureAtLeast(
			TimeSpan.Zero,
			() => new InvalidOperationException("Guaranteed delivery time cannot be negative"));
