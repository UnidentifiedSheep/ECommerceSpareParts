	...
	: BatchableDomainEventHandler<MarkupGroupUpdatedDomainEvent>
{
	public override async Task Handle(
		Batch<MarkupGroupUpdatedDomainEvent> notification,
		CancellationToken cancellationToken)
	{
		var pricingSettings = await settingsService.GetOrDefault<PricingSetting>(cancellationToken);
		if (pricingSettings.Data.SelectedMarkupId != null &&
			notification.Items.All(x => x.Id != pricingSettings.Data.SelectedMarkupId))
			return;
