...
public class ProductPriceOffersUpdatedConsumer(ISender sender)
	: IConsumer<Batch<ProductPriceOffersUpdatedEvent>>
{
	...
	{
		var recalculationRequests = context
			.Message
			.Select(x => new PriceRecalculationRequestDto
			{
				ProductId = x.Message.ProductId, StorageCode = x.Message.StorageCode
			})
			.ToList();
...
public class ProductPriceOffersUpdatedDefinition : ConsumerDefinition<ProductPriceOffersUpdatedConsumer>
{
