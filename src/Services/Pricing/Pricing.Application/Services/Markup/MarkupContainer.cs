...
public class MarkupContainer : IMarkupContainer
{
	...
	{
		EnsureInitialized();
		return _markupMaps.TryGetValue(currencyId, out var map) ? map.Get(value) : null;
	}
		...
		Dictionary<int, IEnumerable<MarkupRange>> other)
	{
		lock (_lock)
		{
			...
			var otherRanges = other.ToDictionary(x => x.Key, x => x.Value.ToList());
	...
	private static void WriteRanges(Utf8JsonWriter writer, IEnumerable<MarkupRange> ranges)
	{
		writer.WriteStartArray();

		foreach (var range in ranges.OrderBy(x => x.RangeStart).ThenBy(x => x.RangeEnd).ThenBy(x => x.Markup))
		{
			writer.WriteStartObject();
	...
	private sealed class MarkupRangeIndex(IndexedMarkupRange[] ranges)
	{
		public Models.Markup? Get(decimal value)
		{
			...
			return candidate >= 0 && value <= ranges[candidate].End ? ranges[candidate].Markup : null;
		}
	}

	private readonly record struct IndexedMarkupRange(decimal Start, decimal End, Models.Markup Markup);
}
