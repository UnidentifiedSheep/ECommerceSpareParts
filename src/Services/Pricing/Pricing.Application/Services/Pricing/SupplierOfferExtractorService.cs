	...
	ISupplierFactory supplierFactory) : ISupplierOfferExtractorService
{
		...
		CancellationToken token)
	{
		try
		{
			return await GetFromSupplierCore(
				supplier,
				productId,
				storageCode,
				token);
		...
		catch (Exception ex)
		{
			logger.LogError(
				ex,
				"Supplier offer extraction failed. Supplier: {Supplier}, ProductId: {ProductId}, Storage: {StorageCode}",
				supplier.Supplier,
				productId,
				storageCode);
			await markerService.MarkAsFailedAsync(
				supplier.Supplier,
				productId,
				storageCode);
	...
	private async Task<SupplierOfferExtractionResult> GetFromSupplierCore(
		...
		CancellationToken token)
	{
		if (await markerService.HasAnyMarkerAsync(
				supplier.Supplier,
				productId,
				storageCode,
				token))
		...
		var result = await distributedLockProvider.TryExecuteWithLock(
			...
			async ct =>
			{
				...
				var mainResponse =
					await mainClient.ProductNode.GetSupplierProductReferences(
						[productId],
						supplier.Supplier,
						ct);
				...
				var response = await supplier.GetProductsAsync(
