	...
	IContextualStringLocalizer localizer) : IPriceApplierService
{
		...
		CancellationToken ct = default)
	{
		...
		var localSystemNames = registeredLocals.Select(x => x.SystemName).ToHashSet();
		var persistedAppliers =
			(await priceApplierProvider.GetConfigurationAsync(ct)).Appliers.ToDictionary(x => x.SystemName);
		var result = new List<PriceApplierDto>();

		foreach (var local in registeredLocals.Where(x => SupportsUsage(x, usage)))
		{
		...
		return result.OrderBy(x => x.States[0].Order).ThenBy(x => x.SystemName).ToList();
	}
	...
	{
		var configuration = await priceApplierProvider.GetConfigurationAsync(ct);
		var applier = configuration.Appliers.FirstOrDefault(x => x.SystemName == systemName);
		...
		CancellationToken ct = default)
	{
		...
		foreach (var info in infos.Where(x => x.States[0].Enabled))
		{
			...
			if (!info.IsDynamic || info.DslLogic is null)
				continue;
			var state = info.States[0];
			result.Add(
				new DynamicApplierNamedObject(
					info.SystemName,
					state.Order,
					info.DslLogic));
		}
	...
	private static bool SupportsUsage(ApplierNamedObjectBase local, PriceOfferSourceType usage)
	{
		return usage switch
		{
