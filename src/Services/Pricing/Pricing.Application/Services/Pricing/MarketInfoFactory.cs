...
public sealed class MarketInfoFactory(IOfferScorer offerScorer, ICurrencyConverter currencyConverter)
	: IMarketInfoFactory
{
		...
		IReadOnlyCollection<CalculatedPriceCandidate> calculatedSupplierCandidates)
	{
		...
		foreach (var candidate in candidates)
		{
			...
			items.Add(
				new MarketInfoItem
				{
					...
					Score = await offerScorer.GetCostScoreAsync(
						new OfferCostScoreContext
						{
		...
		{
			OfferCount = candidates.Length, AvailableQuantity = availableQuantity
		};
	}
}
