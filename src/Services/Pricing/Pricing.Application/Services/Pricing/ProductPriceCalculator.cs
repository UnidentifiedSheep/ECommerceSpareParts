	...
	ISettingsService settingsService) : IProductPriceCalculator
{
	...
	{
		var appliersVersion = await priceApplierService.GetCurrentConfigurationVersionAsync(ct);
		var pricingSettingsVersion = (await settingsService.GetOrDefault<PricingSetting>(ct)).Data.Version;
		...
		var all = supplierCalculated.Concat(internalCalculated).ToList();
