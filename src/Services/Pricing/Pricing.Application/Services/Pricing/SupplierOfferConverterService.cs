	...
	ISettingsService settingsService) : ISupplierOfferConverterService
{
		...
		CancellationToken token = default)
	{
		...
		{
			if (supplierOffer.PurchaseInfo == null)
				continue;
			if (supplierOffer.DeliveryInfo == null)
				continue;

			var currencyId = await cachedCurrencyProvider.GetCurrencyIdAsync(
				supplierOffer.PurchaseInfo.PriceInfo.CurrencyCode,
				token);
