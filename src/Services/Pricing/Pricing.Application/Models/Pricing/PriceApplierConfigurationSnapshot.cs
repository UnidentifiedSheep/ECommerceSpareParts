	...
	string Version)
{
	...
	{
		var appliers = MergeWithLocalAppliers(persistedAppliers, localAppliers);
		var version = ConfigurationVersionGenerator.Generate(writer => WriteConfiguration(writer, appliers));
	...
	private static IReadOnlyList<PriceApplierDto> MergeWithLocalAppliers(
		...
		IReadOnlyCollection<ApplierNamedObjectBase> localAppliers)
	{
		var persistedBySystemName = persistedAppliers.ToDictionary(x => x.SystemName);
		var result = new List<PriceApplierDto>(persistedAppliers.Count + localAppliers.Count);

		foreach (var local in localAppliers)
		{
			persistedBySystemName.Remove(local.SystemName, out var persisted);
	...
	{
		var persistedState = persisted?.States.FirstOrDefault(x => x.Usage == usage);
	...
	private static IEnumerable<PriceOfferSourceType> GetSupportedUsages(ApplierNamedObjectBase local)
	{
	...
	private static void WriteConfiguration(Utf8JsonWriter writer, IReadOnlyList<PriceApplierDto> appliers)
	{
		writer.WriteStartArray();

		foreach (var item in appliers.OrderBy(x => x.SystemName, StringComparer.Ordinal))
		{
