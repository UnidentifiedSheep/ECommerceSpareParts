...
public record GetPriceApplierQuery(string SystemName)
    : IQuery<GetPriceApplierResult>;

public record GetPriceApplierResult(PriceApplierDto Applier);

public class GetPriceApplierHandler(
    ...
    : IQueryHandler<GetPriceApplierQuery, GetPriceApplierResult>
{
    ...
    {
        var applier = await service.FindPriceApplierInfoAsync(
                          request.SystemName,
                          cancellationToken)
