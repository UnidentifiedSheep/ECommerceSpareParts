...
public class DeletePriceApplierHandler(
	...
	IUnitOfWork unitOfWork) : ICommandHandler<DeletePriceApplierCommand>
{
	public async Task<Unit> Handle(DeletePriceApplierCommand request, CancellationToken cancellationToken)
	{
		...
		var applier = await repository.GetById(request.SystemName, cancellationToken) ??
