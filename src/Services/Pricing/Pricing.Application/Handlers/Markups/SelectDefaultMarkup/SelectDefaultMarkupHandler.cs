    ...
    IUnitOfWork unitOfWork) : ICommandHandler<SelectDefaultMarkupCommand>
{
    public async Task<Unit> Handle(SelectDefaultMarkupCommand request, CancellationToken cancellationToken)
    {
        ...
        var selectedMarkupId = setting.SelectedMarkupId;
        ...
        await settingsService.SetSetting(Abstractions.Constants.Settings.Pricing, settingWithNewMarkup,
            cancellationToken);

        await publishEndpoint.Publish(new MarkupGroupChangedEvent { GroupId = request.MarkupGroupId },
            cancellationToken);
