...
{
    public async Task<GetDetailedPriceResult> Handle(
        GetDetailedPricesQuery request,
    ...
    private async Task<decimal> GetUserDiscount(Guid? userId)
    {
        ...
        var discount = await userCacheRepository.GetUserDiscount(userId.Value);
        return discount?.Value ?? 0;
    }

    private async Task<Dictionary<int, BasePricingItemResult?>> GetBasePrices(
