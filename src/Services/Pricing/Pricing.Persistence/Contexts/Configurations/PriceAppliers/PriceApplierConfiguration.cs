...
public class PriceApplierConfiguration : IEntityTypeConfiguration<PriceApplier>
{
	public void Configure(EntityTypeBuilder<PriceApplier> builder)
	{
		...
		builder.HasKey(e => e.SystemName).HasName("price_appliers_pk");

		builder.Property(e => e.SystemName).ValueGeneratedNever().HasColumnName("system_name");

		builder.Property(e => e.Name).HasMaxLength(128).HasColumnName("name");

		builder.Property(e => e.DslLogic).HasColumnType("jsonb").HasColumnName("dsl_logic");
		...
		builder.Navigation(e => e.States).HasField("_states").UsePropertyAccessMode(PropertyAccessMode.Field);
	}
}
