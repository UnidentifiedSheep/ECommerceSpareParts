...
public record GetUserFullInfoResponse(UserInfoDto? UserInfo, List<FullEmailDto> Emails);
        ...
        app.MapGet("/users/{id:guid}/info", async (ISender sender, Guid id, CancellationToken token) =>
            {
                ...
                return Results.Ok(new GetUserFullInfoResponse(info.UserInfo, info.Emails));
            }).WithTags("Users")
