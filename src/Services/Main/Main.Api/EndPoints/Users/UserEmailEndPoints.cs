...
public record AddUserEmailRequest(string Email, EmailType EmailType);
	...
	public static RouteGroupBuilder MapUserEmailEndPoints(this RouteGroupBuilder users)
	{
		users
			.MapPost(
				...
				async (
					...
					AddUserEmailRequest request, CancellationToken cancellationToken) =>
				{
					...
					return Results.Created(
						...
						new AddUserEmailResponse(email.Email));
				})
		...
		users
			.MapGet(
				...
				{
					var result = await sender.Send(new GetUserEmailQuery(userId, email), cancellationToken);
		...
		users
			.MapPost(
				...
				async (
					...
					string email, CancellationToken cancellationToken) =>
				{
					await sender.Send(new RequestEmailVerificationCommand(userId, email), cancellationToken);
		...
		users
			.MapPost(
				...
				{
					await sender.Send(
						new RequestEmailVerificationCommand(userContext.UserId, email),
						cancellationToken);
		...
		users
			.MapPut(
				...
				async (
					...
					string email, CancellationToken cancellationToken) =>
				{
					await sender.Send(new MakeEmailPrimaryCommand(userId, email), cancellationToken);
		...
		users
			.MapPut(
				...
				{
					await sender.Send(
						new MakeEmailPrimaryCommand(userContext.UserId, email),
						cancellationToken);
