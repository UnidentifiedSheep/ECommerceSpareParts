	...
	public void AddRoutes(IEndpointRouteBuilder app)
	{
		...
		users
			.MapPost(
				"/",
				async (
					ISender sender, CreateUserRequest request,
					CancellationToken cancellationToken) =>
				{
					var result = await sender.Send(
						new CreateUserCommand(
							...
							request.Roles),
						cancellationToken);
					var user = await sender.Send(new GetUserFullInfoQuery(result.UserId), cancellationToken);

					return Results.Created($"/users/{result.UserId}", new CreateUserResponse(user.User));
				})
			.WithName("CreateUser")
		...
		users
			.MapGet(
				"/",
				async (
					ISender sender, [AsParameters] GetUsersRequest request,
					IUserContext userContext, CancellationToken token) =>
				{
