...
public class UploadsEndPoints : ICarterModule
{
	...
	{
		var uploads = app.MapGroup("/uploads").WithTags("Uploads");

		uploads
			.MapGet(
				"",
				async (
					ISender sender, string? cursor,
					int size, CancellationToken token) =>
				{
		...
		uploads
			.MapPost(
				"/create",
				async (
					ISender sender, CreateUploadRequest request,
		...
		uploads
			.MapPost(
				"/complete",
				async (
					ISender sender, CompleteUploadRequest request,
					CancellationToken token) =>
				{
