...
public class PurchaseEndPoints : ICarterModule
{
	...
	{
		var purchases = app.MapGroup("/purchases").WithTags("Purchases");

		purchases
			.MapPost(
				"/",
				async (
					ISender sender, CreatePurchaseRequest request,
					CancellationToken token) =>
				{
					...
					var purchase = await sender.Send(new GetPurchaseQuery(result.PurchaseId, null), token);
		...
		purchases
			.MapDelete(
				"/{purchaseId}",
				async (
					ISender sender, Guid purchaseId,
		...
		purchases
			.MapPut(
				...
				async (
					ISender sender, Guid purchaseId,
					EditPurchaseRequest request, CancellationToken cancellationToken) =>
				{
		...
		purchases
			.MapGet(
				...
				async (
					...
					CancellationToken cancellationToken) =>
				{
					var result = await sender.Send(new GetPurchaseQuery(purchaseId, null), cancellationToken);
		...
		purchases
			.MapGet(
				"/",
				async (
					ISender sender, [AsParameters] GetPurchasesRequest request,
					CancellationToken token) =>
				{
