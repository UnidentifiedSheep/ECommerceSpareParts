	...
	public void AddRoutes(IEndpointRouteBuilder app)
	{
		...
		cart
			.MapPost(
				"",
				async (
					ISender sender, IUserContext user,
					AddToCartRequest request, CancellationToken cancellationToken) =>
				{
					await sender.Send(
		...
		cart
			.MapPatch(
				...
				async (
					ISender sender, IUserContext user,
					int articleId, ChangeCartItemCountRequest request,
		...
		cart
			.MapDelete(
				...
				async (
					ISender sender, IUserContext user,
					int articleId, CancellationToken cancellationToken) =>
				{
