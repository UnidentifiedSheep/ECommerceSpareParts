...
public class StoragesEndPoints : ICarterModule
{
	...
	{
		var storages = app.MapGroup("/storages").WithTags("Storages");
		...
		storages
			.MapPost(
				"/",
				async (
					ISender sender, CreateStorageRequest request,
		...
		storages
			.MapDelete(
				"/{storageCode}",
				async (
					ISender sender, string storageCode,
					CancellationToken cancellationToken) =>
				{
		...
		storages
			.MapPatch(
				...
				async (
					ISender sender, string storageCode,
					EditStorageRequest request, CancellationToken token) =>
				{
		...
		storages
			.MapGet(
				"/",
				async (
					ISender sender, int page,
					int limit, string? searchTerm,
					StorageType? type, CancellationToken token) =>
				{
