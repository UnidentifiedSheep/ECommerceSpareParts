	...
	public static RouteGroupBuilder MapProductContentEndPoints(this RouteGroupBuilder products)
	{
		products
			.MapGet(
					...
					CancellationToken token) =>
				{
					...
					var response = new GetProductContentResponse(
						result.Contents.GetValueOrDefault(productId) ?? []);
		...
		products
			.MapPost(
				...
				async (
					ISender sender, int productId,
					AddProductContentRequest request, CancellationToken cancellationToken) =>
				{
		...
		products
			.MapDelete(
				...
				async (
					ISender sender, int productId,
					int childProductId, CancellationToken token) =>
				{
		...
		products
			.MapPatch(
				...
				async (
					ISender sender, int productId,
					int childProductId, SetProductsContentCountRequest request,
					CancellationToken token) =>
				{
