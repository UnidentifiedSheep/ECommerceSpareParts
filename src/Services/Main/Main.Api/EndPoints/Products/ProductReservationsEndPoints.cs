...
public record GetProductReservationResponse(ProductReservationDto Reservation);
	...
	public static RouteGroupBuilder MapProductReservationsEndPoints(this RouteGroupBuilder products)
	{
		products
			.MapPost(
				"/reservations",
				async (
					ISender sender, CreateProductReservationRequest request,
		...
		products
			.MapGet(
				...
				async (
					ISender sender, int reservationId,
					CancellationToken cancellationToken) =>
				{
					var result = await sender.Send(
					...
					return Results.Ok(new GetProductReservationResponse(result.Reservation));
				})
		...
		products
			.MapPut(
				...
				async (
					ISender sender, int reservationId,
					EditProductReservationRequest request, CancellationToken cancellationToken) =>
				{
					await sender.Send(
		...
		products
			.MapGet(
				"/reservations",
				async (
					ISender sender, [AsParameters] GetProductReservationsRequest queryParams,
					CancellationToken cancellationToken) =>
				{
		...
		products
			.MapGet(
				...
				async (
					...
					[AsParameters] PaginationQueryModel queryParams, CancellationToken cancellationToken) =>
				{
					var query = new GetReservationHistoryQuery(id, queryParams);
					var result = await sender.Send(query, cancellationToken);
