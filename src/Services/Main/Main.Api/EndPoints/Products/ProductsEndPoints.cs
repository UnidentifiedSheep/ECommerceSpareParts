	...
	public void AddRoutes(IEndpointRouteBuilder app)
	{
		...
		products
			.MapPost(
				"",
				async (
					ISender sender, CreateProductRequest request,
		...
		products
			.MapGet(
				"",
				async (
					ISender sender, [AsParameters] GetProductByIdsRequest request,
					CancellationToken token) =>
				{
		...
		products
			.MapGet(
				...
				async (
					ISender sender, int productId,
					[FromQuery] string storageCode, CancellationToken token) =>
				{
					var item = new GetAvailableProductsStockItem(productId, storageCode);
					var result = await sender.Send(new GetAvailableProductsStockQuery(item), token);
		...
		products
			.MapPatch(
				...
				async (
					ISender sender, int productId,
					EditProductRequest request, CancellationToken token) =>
				{
