	...
	public void AddRoutes(IEndpointRouteBuilder app)
	{
		...
		currencies
			.MapPost(
				"",
				async (
					ISender sender, CreateCurrencyRequest request,
		...
		currencies
			.MapGet(
				...
				async (ISender sender, CancellationToken cancellation) =>
				{
					var result = await sender.Send(new GetBaseCurrencyQuery(), cancellation);

					return Results.Ok(new GetBaseCurrencyResponse(result.Currency));
				})
		...
		currencies
			.MapGet(
				...
				async (
					ISender sender, int id,
					[AsParameters] PaginationQueryModel queryParams, CancellationToken cancellation) =>
				{
					var result = await sender.Send(
