	...
	public void AddRoutes(IEndpointRouteBuilder app)
	{
		...
		auth
			.MapPost(
				"/password/",
				async (
					ISender sender, ChangePasswordRequest request,
		...
		auth
			.MapPost(
				"/password/recovery",
				async (
					ISender sender, SendEmailRecoveryRequest request,
					CancellationToken cancellationToken) =>
				{
					await sender.Send(new SendEmailRecoveryCommand(request.Email), cancellationToken);
		...
		auth
			.MapPost(
				"/password/reset",
				async (
					ISender sender, ResetPasswordRequest request,
					CancellationToken cancellationToken) =>
				{
					await sender.Send(
		...
		auth
			.MapGet(
				...
				async (
					ISender sender, string userName,
					CancellationToken cancellationToken) =>
				{
					var result = await sender.Send(new IsUserNameAvailableQuery(userName), cancellationToken);
		...
		auth
			.MapPost(
				"/login",
				async (
					LoginRequest request, ISender sender,
		...
		auth
			.MapPost(
				"/refresh",
				async (
					ISender sender, RefreshTokenRequest request,
		...
		auth
			.MapPost(
				"/register/",
				async (
					RegisterRequest request, ISender sender,
					CancellationToken ct) =>
				{
					await sender.Send(
						new RegisterCommand(
