	...
	public static RouteGroupBuilder MapTransactionEndPoints(this RouteGroupBuilder balances)
	{
		balances
			.MapPost(
				"",
				async (
					ISender sender, CreateTransactionRequest request,
		...
		balances
			.MapDelete(
				...
				async (
					...
					[FromQuery] bool forcePayment = false, CancellationToken token = default) =>
				{
					await sender.Send(
		...
		balances
			.MapGet(
				"",
				async (
					ISender sender, [AsParameters] GetTransactionsRequest request,
					CancellationToken token) =>
				{
					var query = new GetTransactionsQuery(
						...
						new Cursor<(Guid id, DateTime dt)>(
							(request.CursorId ?? Guid.Empty, request.CursorDate ?? DateTime.MinValue),
							request.Size),
