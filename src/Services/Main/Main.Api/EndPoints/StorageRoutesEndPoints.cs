...
public class StorageRoutesEndPoints : ICarterModule
{
	...
	{
		var routes = app.MapGroup("/storages/routes").WithTags("Storage Routes");

		routes
			.MapPost(
				"",
				async (
					ISender sender, AddStorageRouteRequest request,
		...
		routes
			.MapGet(
				"",
				async (
					ISender sender, [AsParameters] GetStorageRoutesRequest request,
					CancellationToken token) =>
				{
