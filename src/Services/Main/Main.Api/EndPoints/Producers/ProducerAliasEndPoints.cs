	...
	public static RouteGroupBuilder MapProducerAliasEndPoints(this RouteGroupBuilder producers)
	{
		producers
			.MapPost(
				...
				async (
					...
					AddAliasToProducerRequest request, CancellationToken token) =>
				{
					await sender.Send(new AddAliasCommand(producerId, request.Alias), token);
					return Results.Ok();
				})
		...
		producers
			.MapGet(
				...
				async (
					ISender sender, int producerId,
					[AsParameters] PaginationQueryModel request, CancellationToken token) =>
				{
