...
public record CreateProducerSupplierMappingRequest(Supplier Supplier, string SupplierProducerName);
	...
	public static RouteGroupBuilder MapProducerSupplierMappingEndPoints(this RouteGroupBuilder producers)
	{
		producers
			.MapPost(
				...
				async (
					...
					CreateProducerSupplierMappingRequest request, CancellationToken token) =>
				{
		...
		producers
			.MapDelete(
				...
				async (
					ISender sender, int producerId,
					int mappingId, CancellationToken cancellationToken) =>
				{
		...
		producers
			.MapGet(
				...
				async (
					ISender sender, int producerId,
					[AsParameters] GetProducerSupplierMappingsRequest request, CancellationToken token) =>
				{
					var result = await sender.Send(
