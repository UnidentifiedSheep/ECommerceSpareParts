...
public class PurchaseContent : Entity<PurchaseContent, int>, ILinqEntity<PurchaseContent, int>
{
	...
	public static Expression<Func<PurchaseContent, int>> GetKeySelector() => x => x.Id;

	public static Expression<Func<PurchaseContent, bool>> GetEqualityExpression(int key) => x => x.Id == key;
	...
	public void SetPrice(decimal price)
	{
		Price = price
			...
			.EnsureGreaterThan(0, () => new InvalidOperationException("Price must be greater than zero."));
		CalculateTotalSum();
	}
	...
	{
		Comment = comment.NullIfWhiteSpace()?.EnsureMaxLength(256, "purchase.content.comment.too.long");
	}
