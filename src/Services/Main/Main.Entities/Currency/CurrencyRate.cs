...
public class CurrencyRate : AuditableEntity<CurrencyRate, (int, int)>, ILinqEntity<CurrencyRate, (int, int)>
{
	...
	public static Expression<Func<CurrencyRate, (int, int)>> GetKeySelector() => x =>
		ValueTuple.Create(x.FromCurrencyId, x.ToCurrencyId);

	public static Expression<Func<CurrencyRate, bool>> GetEqualityExpression((int, int) key) => x =>
		x.FromCurrencyId == key.Item1 && x.ToCurrencyId == key.Item2;
