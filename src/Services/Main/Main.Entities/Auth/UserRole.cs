...
public class UserRole : AuditableEntity<UserRole, (Guid, string)>, ILinqEntity<UserRole, (Guid, string)>
{
	...
	public static Expression<Func<UserRole, (Guid, string)>> GetKeySelector() => x =>
		ValueTuple.Create(x.UserId, x.RoleName);
	...
	public static UserRole Create(Guid userId, string roleName) => new(userId, roleName);
