...
public sealed class ProducerSupplierMappingImportLrtTests
    : LrtIntegrationTest<ProducerSupplierMappingImportLrt>
{
    private const string UploadsBucket = "uploads";

    public ProducerSupplierMappingImportLrtTests(
    ...
    public async Task DifferentNamesForSameProducerAndSupplier_AreInserted()
    {
        ...
        var execution = await ExecuteCsv(
            CsvRow(producer.Name, Supplier.Armtek, "Bosch"),
            CsvRow(producer.Name, Supplier.Armtek, "Bosch Automotive"));

        execution.Job.Status.Should().Be(
            JobStatus.Succeeded,
            execution.Job.ErrorMessage);
    ...
    public async Task ExistingSupplierName_IsNotRemappedToAnotherProducer()
    {
        ...
        var persisted = await Context.ProducerSupplierMappings
            .AsNoTracking()
            .SingleAsync();
    ...
    public async Task InvalidAndDuplicateRows_AreReported_AndValidMappingIsTrimmed()
    {
        ...
        var execution = await ExecuteCsv(
            CsvRow(producer.Name, Supplier.Armtek, " Bosch "),
            CsvRow(producer.Name, Supplier.Armtek, "Bosch"),
            CsvRow("Unknown producer", Supplier.Armtek, "Unknown"),
            CsvRow(producer.Name, Supplier.Armtek, " "));
        ...
        params string[] rows)
    {
        ...
        var csv = string.Join(
            Environment.NewLine,
            new[] { "Producer,Supplier,SupplierProducer" }.Concat(rows));
    ...
    {
        return $"{Escape(producer)},{supplier},{Escape(supplierProducerName)}";
    }
