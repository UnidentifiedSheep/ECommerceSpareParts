...
public sealed class ProducerAliasImportLrtTests : CsvLrtIntegrationTest<ProducerAliasImportLrt>
{
	public ProducerAliasImportLrtTests(CombinedContainerFixture fixture) : base(fixture)
	{
	...
	public async Task InvalidAndDuplicateAliases_AreReported_AndValidAliasIsCreated()
	{
		...
		var execution = await ExecuteCsv(
			...
			fileName => new ProducerAliasesImportInputState
			{
				FileName = fileName
			});
		...
		var aliases = await Context.ProducersAliases.AsNoTracking().ToListAsync();
