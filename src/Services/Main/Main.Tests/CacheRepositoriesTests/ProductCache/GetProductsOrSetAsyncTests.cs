...
public class GetProductsOrSetAsyncTests : IntegrationTest
{
	...
	public async Task GetProductsOrSetAsync_WhenSomeProductsAlreadyCached_ReturnsCachedAndDbProducts()
	{
		...
		rawCached.Should().ContainSingle().Which.Should().BeEquivalentTo(result[dbProduct.Id]);
	}
	...
	private Task RemoveCachedProducts(IEnumerable<int> productIds) =>
		Task.WhenAll(productIds.Select(RemoveCachedProduct));
	...
	{
		var producer = TestContext.ProducerTestContext.Producers.First(x => x.Id == product.ProducerId);
