...
public abstract class CsvLrtIntegrationTest<TLrt>(CombinedContainerFixture fixture)
	: LrtIntegrationTest<TLrt>(fixture) where TLrt : class, ILrtNamedObject
{
	...
	protected Task<LrtExecutionResult> ExecuteCsv<TInputState>(
		...
		Func<string, TInputState> createState) where TInputState : ICsvImportInputState
	{
	...
	protected static string CsvRow(params object?[] values) => string.Join(
		",",
		values.Select(x => Escape(x?.ToString() ?? string.Empty)));
