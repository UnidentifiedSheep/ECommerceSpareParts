...
public sealed class CatalogueCandidateTests
{
    ...
    {
        var action = () => CatalogueCandidate.Create(
            "ABC-123",
            producerId);

        action.Should()
            .Throw<ArgumentOutOfRangeException>()
            .Which.ParamName.Should().Be("producerId");
    }
    ...
    public void MapToProduct_InvalidProductId_Throws(int productId)
    {
        ...
        action.Should()
            .Throw<ArgumentOutOfRangeException>()
            .Which.ParamName.Should().Be("productId");
        candidate.ProductId.Should().BeNull();
    }
    ...
    public void AddSupplierProduct_NewProduct_AddsProductAndUpdatedEvent()
    {
        ...
        candidate.SupplierProducts.Should().ContainSingle()
            .Which.Should().BeSameAs(supplierProduct);
        candidate.FlushDomainEvents().Should().ContainSingle()
            .Which.Should().Be(
    ...
    public void AddSupplierProduct_NullProduct_Throws()
    {
        ...
        action.Should()
            ...
            .Which.ParamName.Should().Be("supplierProduct");
        candidate.SupplierProducts.Should().BeEmpty();
        candidate.FlushDomainEvents().Should().BeEmpty();
    }
    ...
    public void AddSupplierProduct_MultipleProducts_AddsSingleUpdatedEvent()
    {
        ...
        candidate.SupplierProducts.Should().Equal(
            firstProduct,
            secondProduct);
        candidate.FlushDomainEvents().Should().ContainSingle()
            .Which.Should().Be(
    ...
    public void GetKeySelector_ReturnsCandidateId()
    {
        ...
        var id = CatalogueCandidate.GetKeySelector()
            .Compile()(candidate);
    ...
    {
        var candidate = CreateCandidate();
        var predicate = CatalogueCandidate
            .GetEqualityExpression(candidate.Id)
            .Compile();
    ...
    public void GetEqualityExpression_DifferentId_ReturnsFalse()
    {
        var candidate = CreateCandidate();
        var predicate = CatalogueCandidate
            .GetEqualityExpression(Guid.CreateVersion7())
            .Compile();
    ...
    public void OnCreated_AddsCreatedEvent()
    {
        ...
        var domainEvent = candidate.FlushDomainEvents()
            .Should().ContainSingle()
            .Which.Should().BeOfType<EntityCreatedDomainEvent<CatalogueCandidate>>()
            .Which;
    ...
    public void OnUpdated_AddsUpdatedEvent()
    {
        ...
        candidate.FlushDomainEvents().Should().ContainSingle()
            .Which.Should().Be(
                new EntityUpdatedDomainEvent<CatalogueCandidate, Guid>(candidate.Id));
    }
    ...
    public void OnDeleted_AddsDeletedEvent()
    {
        ...
        candidate.FlushDomainEvents().Should().ContainSingle()
            .Which.Should().Be(
                new EntityDeletedDomainEvent<CatalogueCandidate, Guid>(candidate.Id));
    }

    private static CatalogueCandidate CreateCandidate()
    {
        return CatalogueCandidate.Create("ABC-123", 42);
    }

    private static SupplierProduct CreateSupplierProduct(
        string sku = "SUPPLIER-123")
    {
