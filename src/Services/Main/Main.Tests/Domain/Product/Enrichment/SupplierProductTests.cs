...
public sealed class SupplierProductTests
{
	...
	public void AddName_MultipleNames_AddsSingleContentChangedEvent()
	{
		...
		supplierProduct.Names.Select(x => x.Name).Should().Equal("Oil filter", "Air filter");
		supplierProduct
			.FlushDomainEvents()
			.Should()
			.ContainSingle()
	...
	public void GetKeySelector_ReturnsSupplierProductId()
	{
		...
		var id = SupplierProduct.GetKeySelector().Compile()(supplierProduct);
	...
	{
		var supplierProduct = CreateSupplierProduct();
		var predicate = SupplierProduct.GetEqualityExpression(supplierProduct.Id).Compile();
	...
	{
		var supplierProduct = CreateSupplierProduct();
		var predicate = SupplierProduct.GetEqualityExpression(42).Compile();
	...
	private static SupplierProduct CreateSupplierProduct(Guid? catalogueCandidateId = null)
	{
		var supplierProduct = SupplierProduct.Create(
			new Sku("SUPPLIER-123"),
			"Supplier producer",
		...
		if (catalogueCandidateId.HasValue)
			typeof(SupplierProduct).GetProperty(nameof(SupplierProduct.CatalogueCandidateId))!.SetValue(
				supplierProduct,
	...
	private static CatalogueCandidate CreateCandidate() => CatalogueCandidate.Create("CANDIDATE-123", 42);
}
