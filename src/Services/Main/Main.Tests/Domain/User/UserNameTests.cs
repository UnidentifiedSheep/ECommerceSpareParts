...
public class UserNameTests
{
		...
		string expectedMessageKey)
	{
		...
		action.Should().Throw<InvalidInputException>().Which.MessageKey.Should().Be(expectedMessageKey);
	}
	...
	public void Constructor_WhenValueExceedsMaximumLength_ThrowsInvalidInputException()
	{
		...
		action.Should().Throw<InvalidInputException>().Which.MessageKey.Should().Be("login.max.length.36");
	}
