...
public class OrganizationMemberTests
{
	...
	public void GetEqualityExpression_WithMatchingKey_ReturnsTrue()
	{
		...
		var result = OrganizationMember.GetEqualityExpression(member.GetId()).Compile()(member);
	...
	public void GetEqualityExpression_WithDifferentKey_ReturnsFalse()
	{
		var member = OrganizationMember.Create(
			...
			Guid.NewGuid(),
			OrganizationRole.Member);
		var differentKey = new OrganizationMemberKey(member.OrganizationId, Guid.NewGuid());

		var result = OrganizationMember.GetEqualityExpression(differentKey).Compile()(member);
