...
public class SendLoginNotificationEmailHandlerTests(CombinedContainerFixture fixture)
	: IntegrationTest(fixture)
{
	...
	public async Task Handle_LoadsPrimaryEmailsAndQueuesMessagesAsBatch()
	{
		...
		renderer
			.Setup(x => x.RenderAsync(It.IsAny<LoginNotificationData>(), It.IsAny<CancellationToken>()))
			.ReturnsAsync((LoginNotificationData data, CancellationToken _) =>
				new EmailMessage(
					data.Subject,
					data.To,
					"body"));
		...
		mailingService
			...
			.Callback((IEnumerable<IEmailMessage> emails, CancellationToken _) =>
				queuedEmails = emails.ToList())
		...
		queuedEmails!
			.Select(x => x.To)
			.Should()
			.BeEquivalentTo("first-primary@example.com", "second-primary@example.com");
		mailingService.Verify(
			x => x.QueueEmailAsync(It.IsAny<IEnumerable<IEmailMessage>>(), It.IsAny<CancellationToken>()),
			Times.Once);
	}
}
