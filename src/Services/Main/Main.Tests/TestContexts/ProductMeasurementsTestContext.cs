...
public class ProductMeasurementsTestContext(DContext context, ProductTestContext productContext)
	: TestContextBase<DContext>(context), IDependentTestContext
{
	...
	public static Type[] DependsOn { get; } = [typeof(ProductTestContext)];
	...
	public override async Task InitializeAsync(CancellationToken cancellationToken = default)
	{
		...
		{
			sizeBuilders.Add(new ProductSizeBuilder(Faker).WithProductId(product.Id));

			weightsBuilder.Add(new ProductWeightBuilder(Faker).WithProductId(product.Id));
		}
