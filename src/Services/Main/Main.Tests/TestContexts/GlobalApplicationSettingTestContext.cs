...
public abstract class GlobalApplicationSettingTestContext(
	...
	TestSystemOptionsAccessor systemOptionsAccessor) : TestContextBase<DContext>(context)
{
	public User SystemUser { get; private set; } = null!;

	public override async Task InitializeAsync(CancellationToken cancellationToken = default)
	{
		SystemUser = new SystemUserBuilder(Faker).WithUserName("SYSTEM").Build();
		...
		var setting = new GlobalApplicationSetting(
			...
			{
				ApiServiceUrl = "https://www.somewebsite.com", AppServiceUrl = "https://www.somewebsite.com"
			});
