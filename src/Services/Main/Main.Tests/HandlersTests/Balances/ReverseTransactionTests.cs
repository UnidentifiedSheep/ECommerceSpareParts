...
public class ReverseTransactionTests : IntegrationTest
{
	...
	public async Task ReverseTransaction_ValidData_Succeeds()
	{
		...
		var reversed = await Context.Transactions.AsNoTracking().FirstAsync(x => x.Id == transaction.Id);
	...
	private async Task<Transaction> CreateAppliedTransaction(TransactionSourceType sourceType)
	{
		...
		var senderBalance =
			await Context.UserBalances.FirstAsync(x =>
				x.OrganizationId == senderId && x.CurrencyId == currencyId);
		var receiverBalance = await Context.UserBalances.FirstAsync(x =>
			x.OrganizationId == receiverId && x.CurrencyId == currencyId);
