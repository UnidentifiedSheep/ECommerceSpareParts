...
public class RemoveEmailFromUserTests(CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
	[Fact]
	public async Task RemoveEmail_SecondaryEmail_RemovesEmail()
	{
		...
		await Mediator.Send(new RemoveEmailFromUserCommand(user.Id, "secondary@example.com"), CancellationToken);
	...
	private async Task<User> CreateUser()
	{
		...
		builder.WithEmail("primary@example.com", isPrimary: true).WithEmail("secondary@example.com");
