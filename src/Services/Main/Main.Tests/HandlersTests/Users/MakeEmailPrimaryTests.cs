...
public class MakeEmailPrimaryTests(
    CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
    ...
    public async Task MakeEmailPrimary_ConfirmedEmail_ReplacesCurrentPrimary()
    {
        ...
        emails.Single(x => x.IsPrimary)
            .Email.Value.Should().Be("new-primary@example.com");
    }
    ...
    public async Task MakeEmailPrimary_UnconfirmedEmail_Throws()
    {
        ...
        exception.Which.MessageKey.Should().Be(
    ...
    private async Task<Main.Entities.User.User> CreateUser(
        bool newEmailConfirmed = true)
    {
        var user = await new MemberUserBuilder(Faker)
            .WithEmail(
                "current-primary@example.com",
                isPrimary: true)
            .WithEmail(
                "new-primary@example.com",
                isConfirmed: newEmailConfirmed)
