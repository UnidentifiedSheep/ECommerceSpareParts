...
public class CreatePurchaseTests : IntegrationTest
{
	...
	public override async ValueTask InitializeAsync()
	{
		...
		_supplier = await new SupplierUserBuilder(Faker).BuildAndAddToDb(Context);
	}
	...
	public async Task CreatePurchase_WithoutLogistics_CreatesPurchaseAndStorageContents()
	{
		...
		var storageContents = await Context.StorageContents.AsNoTracking().ToListAsync(cancellationToken: CancellationToken);
		storageContents.Should().HaveCount(command.PurchaseContent.Count());
		foreach (var item in command.PurchaseContent)
			storageContents
				.Should()
				.Contain(x =>
					x.ProductId == item.ProductId && x.Count == item.Count && x.BuyPrice == item.Price &&
					x.CurrencyId == command.CurrencyId && x.StorageCode == command.StorageCode);

		var purchaseTransaction = await Context
			.Transactions
			.AsNoTracking()
			.SingleAsync(x => x.Id == purchase.TransactionId, cancellationToken: CancellationToken);
	...
	public async Task CreatePurchase_ForBusinessOrganization_UsesOrganizationForTransaction()
	{
		...
		var purchase = await Context.Purchases.AsNoTracking().SingleAsync(x => x.Id == result.PurchaseId, cancellationToken: CancellationToken);
		purchase.SupplierUserId.Should().Be(_supplier.Id);
	...
	public async Task CreatePurchase_WithLogistics_CreatesPurchaseLogisticsAndContentLogistics()
	{
		...
		var product = GetContext<ProductTestContext>().Products.First();
		await AddLogisticsDependencies(
			product,
			route,
			_supplier.Id);
		...
		if (route.CarrierId is not null)
		{
			...
			var logisticsTransaction = await Context
				.Transactions
				.AsNoTracking()
				.SingleAsync(x => x.Id == purchase.PurchaseLogistic.TransactionId, cancellationToken: CancellationToken);
	...
	public async Task CreatePurchase_WithPayedSum_CreatesPaymentTransaction()
	{
		...
		var transactions = await Context.Transactions.AsNoTracking().ToListAsync(cancellationToken: CancellationToken);
	...
	public async Task CreatePurchase_WithOverpaymentAndForcePayment_CreatesPurchase()
	{
		var command = CreateValidCommand() with
		{
			PayedSum = 9_999m, ForcePayment = true
		};

		await Mediator.Send(command, CancellationToken);

		var supplierBalance = await Context
			.UserBalances
			.AsNoTracking()
			.SingleAsync(x =>
				x.OrganizationId == command.SupplierOrganizationId && x.CurrencyId == command.CurrencyId, cancellationToken: CancellationToken);
	...
	[Fact]
	public async Task
		CreatePurchase_WithLogisticsButNoContentMarkedForLogistics_DoesNotCreatePurchaseLogistic()
	{
		var currency = GetContext<CurrencyTestContext>().Currencies[0];
		var route = GetContext<StorageRouteTestContext>().ActiveRoute;
		var product = GetContext<ProductTestContext>().Products.First();
		...
		var purchase = await Context.Purchases.Include(x => x.PurchaseLogistic).AsNoTracking().SingleAsync(cancellationToken: CancellationToken);
	...
	public async Task CreatePurchase_WithInvalidContentCount_ThrowsValidationException(int count)
	{
		...
		var content = command.PurchaseContent.Single() with
		{
			Count = count
		};
	...
	public async Task CreatePurchase_WithEmptySupplierOrganizationId_ThrowsValidationException()
	{
		var command = CreateValidCommand() with
		{
			SupplierOrganizationId = Guid.Empty
		};
	...
	public async Task CreatePurchase_WithMissingSupplierOrganization_ThrowsDbValidationException()
	{
		var command = CreateValidCommand() with
		{
			SupplierOrganizationId = Guid.NewGuid()
		};
	...
	public async Task CreatePurchase_WithLogisticsAndEmptyStorageFrom_ThrowsValidationException()
	{
		var command = CreateValidCommand() with
		{
			WithLogistics = true, StorageFrom = null
		};
	...
	public async Task CreatePurchase_WithMissingStorage_ThrowsDbValidationException()
	{
		var command = CreateValidCommand() with
		{
			StorageCode = Faker.Lorem.Letter(200)
		};
	...
	public async Task CreatePurchase_WithMissingProduct_ThrowsProductNotFoundException()
	{
		...
		await Assert.ThrowsAsync<ProductNotFoundException>(() => Mediator.Send(command with
			{
				PurchaseContent = [content]
			}, CancellationToken));
	}
	...
	public async Task CreatePurchase_WithLogisticsAndMissingProductSize_ThrowsProductSizesNotFoundException()
	{
		...
		await Assert.ThrowsAsync<ProductSizesNotFoundException>(() => Mediator.Send(command, CancellationToken));
	}

	[Fact]
	public async Task
		CreatePurchase_WithLogisticsAndMissingProductWeight_ThrowsProductWeightNotFoundException()
	{
		var route = GetContext<StorageRouteTestContext>().ActiveRoute;
		var product = GetContext<ProductTestContext>().Products.First();
		Context.ProductSizes.Add(
			ProductSize.Create(
				product.Id,
				1m,
				1m,
		...
		await Assert.ThrowsAsync<ProductWeightNotFoundException>(() => Mediator.Send(command, CancellationToken));
	}
	...
	private static NewPurchaseContentDto NewContent(
		Product product,
		int count,
		bool calculateLogistics)
	{
	...
	private async Task AddLogisticsDependencies(
		Product product,
		StorageRoute route,
		Guid supplierId)
	{
