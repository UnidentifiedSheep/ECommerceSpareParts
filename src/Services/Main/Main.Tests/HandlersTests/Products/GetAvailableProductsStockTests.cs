...
public class GetAvailableProductsStockTests : IntegrationTest
{
    ...
    {
        var items = TestContext.StorageContents
            .Select(x => new GetAvailableProductsStockItem(
                x.ProductId,
                x.StorageCode))
            .Distinct()
            .Take(3)
        ...
        var result = await Mediator.Send(
            new GetAvailableProductsStockQuery(items));
    ...
    public async Task GetAvailableProductsStock_WithDuplicateItems_ReturnsSingleStock()
    {
        ...
        var result = await Mediator.Send(
            new GetAvailableProductsStockQuery([item, item]));
