...
public class ImportSupplierProductTests(CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
	[Fact]
	public async Task Import_DuplicateProductsInBatch_MergesNames()
	{
		...
		products[0].Names.Select(x => x.Name).Should().BeEquivalentTo("Filter", "Oil filter");
	}
	...
	public async Task Import_DifferentSupplierBrandSpellings_RemainDistinct()
	{
		...
		products.Select(x => x.Producer).Should().BeEquivalentTo("Bosch", "BOSCH");
	}
	...
	public async Task Import_Analogues_ImportsProductsNamesAndCrosses()
	{
		...
		products.Should().HaveCount(2);
		products
			.Single(x => x.Producer == "Bosch")
			.Names
			.Select(x => x.Name)
			.Should()
			.ContainSingle("Product name");
		products
			.Single(x => x.Producer == "MANN")
			.Names
			.Select(x => x.Name)
			.Should()
	...
	private async Task<List<SupplierProduct>> GetProducts()
	{
