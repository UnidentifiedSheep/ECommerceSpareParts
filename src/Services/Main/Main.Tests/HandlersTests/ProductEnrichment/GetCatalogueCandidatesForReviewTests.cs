...
{
    public GetCatalogueCandidatesForReviewTests(CombinedContainerFixture fixture)
        : base(fixture)
    {
    ...
    public async Task ExistingCandidate_ReturnsCompleteReviewProjection()
    {
        ...
        var projectedSupplierProduct = projectedCandidate.SupplierProducts
    ...
    public async Task ProductIdSpecified_ReturnsOnlyMappedCandidate()
    {
        ...
        var result = await Mediator.Send(
            CreateQuery(productId: TestContext.Product.Id));
    ...
    public async Task ProductIdAndSkuSpecified_AppliesBothFilters()
    {
        var result = await Mediator.Send(
            CreateQuery(
                productId: TestContext.Product.Id,
                sku: "missing-sku"));

        result.Candidates.Should().BeEmpty();
    }

    private static GetCatalogueCandidatesForReviewQuery CreateQuery(
        int? productId = null,
        string? sku = null)
    {
