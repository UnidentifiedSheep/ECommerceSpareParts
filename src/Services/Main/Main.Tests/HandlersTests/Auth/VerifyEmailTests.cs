...
public class VerifyEmailTests : IntegrationTest
{
	...
	public VerifyEmailTests(CombinedContainerFixture fixture) : base(fixture)
	{
	...
	public async Task VerifyEmail_WrongVerificationType_DoesNotConsumeToken()
	{
		...
		await action.Should().ThrowAsync<EmailVerificationTokenExpiredException>();
		(await GetPayloadProvider().TryConsumeToken(payload.Id)).Should().BeTrue();
	}
	...
	{
		await CreateUser();
		var anotherUser = await CreateUser(null);
		var (payload, token) = await CreateToken(
			anotherUser.Id,
			VerificationType.EmailVerification,
			Email);
	...
	private async Task<User> CreateUser(string? email = Email)
	{
		var builder = new MemberUserBuilder(Faker);
		if (email is not null)
			builder.WithEmail(email, isConfirmed: false);
