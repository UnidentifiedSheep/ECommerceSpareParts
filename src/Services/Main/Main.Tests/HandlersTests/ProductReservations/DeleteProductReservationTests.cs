...
public class DeleteProductReservationTests : IntegrationTest
{
	...
	public ProductReservationTestContext TestContext => GetContext<ProductReservationTestContext>();
	...
	public async Task WhenNotCancelled_CancelsReservation()
	{
		...
		var dbValue = await Context
			.ProductReservations
			.AsNoTracking()
			.FirstOrDefaultAsync(x => x.Id == command.ReservationId, cancellationToken: CancellationToken);
	...
	{
		var command = new DeleteProductReservationCommand(TestContext.CanceledReservation.Id);
		var act = () => Mediator.Send(command);
