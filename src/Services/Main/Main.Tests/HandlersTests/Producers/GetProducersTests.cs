...
public class GetProducersTests(CombinedContainerFixture fixture) : IntegrationTest(fixture)
{
	...
	public async Task GetProducers_WithMultipleIds_ReturnsOnlyRequestedProducers()
	{
		...
		var result = await Mediator.Send(CreateQuery([first.Id, second.Id]));

		result.Producers.Select(x => x.Id).Should().BeEquivalentTo([first.Id, second.Id]);
	...
	{
		var first = await new ProducerBuilder(Faker).WithName("First producer").BuildAndAddToDb(Context);
		var second = await new ProducerBuilder(Faker).WithName("Second producer").BuildAndAddToDb(Context);
	...
	public async Task GetProducers_WithDuplicateAndMissingIds_ReturnsExistingProducerOnce()
	{
		...
		var result = await Mediator.Send(CreateQuery([producer.Id, producer.Id, int.MaxValue]));
	...
	{
		var first = await new ProducerBuilder(Faker).WithName("Alpha producer").BuildAndAddToDb(Context);
		var second = await new ProducerBuilder(Faker).WithName("Beta producer").BuildAndAddToDb(Context);
