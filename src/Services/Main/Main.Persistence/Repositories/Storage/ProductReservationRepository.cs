		...
		IProductReservationRepository
{
		...
		CancellationToken cancellationToken = default)
	{
		return GetReservationsCountInternalAsync(
			r => r.OrganizationId == organizationId && productIds.Contains(r.ProductId) &&
				(r.Status == ProductReservationStatus.Active || r.Status == ProductReservationStatus.Locked),
			cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		return Context
			...
			.Select(g => new
			{
				ProductId = g.Key, TotalCount = g.Sum(y => y.CurrentCount)
			})
			.ToDictionaryAsync(
				x => x.ProductId,
				x => x.TotalCount,
