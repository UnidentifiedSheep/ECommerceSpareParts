...
public class SupplierProductRepository(
    ...
    : LinqRepositoryBase<DContext, SupplierProduct, int>(context, extensions),
        ISupplierProductRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var normalizedSkus = requestedKeys
            ...
            .Distinct()
            .ToList();
        var producers = requestedKeys
            .Select(x => x.Producer)
            .Distinct()
            .ToList();

        var products = await Context.SupplierProducts
            .Include(x => x.Names)
        ...
        return products
            .Where(x => requestedKeys.Contains((x.Sku.NormalizedValue, x.Producer)))
            .ToList();
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await Context.BulkInsertOrUpdateAsync(
            ...
            new BulkConfig
            {
                UpdateByProperties =
                [
                    nameof(SupplierProductCross.LeftId),
                    nameof(SupplierProductCross.RightId)
                ]
            },
