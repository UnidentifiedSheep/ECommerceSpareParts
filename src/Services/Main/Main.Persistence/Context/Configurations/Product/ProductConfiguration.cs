...
public class ProductConfiguration : IEntityTypeConfiguration<Entities.Product.Product>
{
	public void Configure(EntityTypeBuilder<Entities.Product.Product> builder)
	{
		...
		builder.HasIndex(e => e.PairId, "products_pair_id_index").IsUnique();
		...
		builder.HasComplexCompositeIndex(
			x => new
			{
				x.Sku.NormalizedValue, x.ProducerId
			},
		...
		builder.ComplexProperty(
			...
			b =>
			{
				b.Property(e => e.Value).HasColumnName("sku");

				b
					.Property(e => e.NormalizedValue)
					.HasColumnName("normalized_sku")
					.HasComplexIndex(e =>
					{
						e.UseGin().HasName("products_sku_index").HasOperators("gin_trgm_ops");
					});
			});
		...
		builder.OwnsOne(
			...
			b =>
			{
				b.Property(e => e.Value).HasColumnName("name").HasMaxLength(255);
			});
		...
		builder.Property(e => e.PackingUnit).HasColumnName("packing_unit");

		builder.Property(e => e.Popularity).HasDefaultValue(1L).HasColumnName("popularity");
		...
		builder
			.Navigation(e => e.Characteristics)
			.HasField("_characteristics")
			.UsePropertyAccessMode(PropertyAccessMode.Field);

		builder.Navigation(e => e.Eans).HasField("_eans").UsePropertyAccessMode(PropertyAccessMode.Field);

		builder.Navigation(e => e.Images).HasField("_images").UsePropertyAccessMode(PropertyAccessMode.Field);
	}
}
