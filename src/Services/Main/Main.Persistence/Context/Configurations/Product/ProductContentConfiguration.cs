...
public class ProductContentConfiguration : IEntityTypeConfiguration<ProductContent>
{
	public void Configure(EntityTypeBuilder<ProductContent> builder)
	{
		...
		builder
			.HasKey(e => new
			{
				e.ParentProductId, e.ChildProductId
			})
			.HasName("product_contents_pk");

		builder.HasIndex(e => e.ChildProductId).HasDatabaseName("product_contents_child_id_idx");

		builder.Property(e => e.ParentProductId).HasColumnName("parent_product_id");

		builder.Property(e => e.ChildProductId).HasColumnName("child_product_id");
