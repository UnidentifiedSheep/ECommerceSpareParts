...
public class ProductCrossConfiguration : IEntityTypeConfiguration<ProductCross>
{
	public void Configure(EntityTypeBuilder<ProductCross> builder)
	{
		...
		builder.Property(x => x.LeftProductId).HasColumnName("left_product_id").IsRequired();
		builder.Property(x => x.RightProductId).HasColumnName("right_product_id").IsRequired();

		builder.HasIndex(e => e.RightProductId).HasDatabaseName("product_crosses_right_id_idx");
	}
}
