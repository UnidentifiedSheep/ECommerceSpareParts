...
public class CatalogueCandidateConfiguration : IEntityTypeConfiguration<CatalogueCandidate>
{
	public void Configure(EntityTypeBuilder<CatalogueCandidate> builder)
	{
		builder.ToTable("catalogue_candidates", "catalogue_enrichment");

		builder.HasKey(x => x.Id).HasName("catalogue_candidates_pk");
		...
		builder.Property(x => x.ProducerId).HasColumnName("producer_id").IsRequired();

		builder.Property(x => x.ProductId).HasColumnName("product_id");

		builder
			.HasOne<ProducerEntity>(e => e.Producer)
			.WithMany()
		...
		builder
			.HasOne<ProductEntity>(e => e.Product)
			.WithOne()
		...
		builder
			...
			.HasDatabaseName("catalogue_candidates_product_id_uidx")
			.IsUnique()
			.HasFilter("product_id IS NOT NULL");

		builder
			.Navigation(x => x.SupplierProducts)
