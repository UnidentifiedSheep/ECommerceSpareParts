...
public class OrganizationMemberConfiguration : IEntityTypeConfiguration<OrganizationMember>
{
	public void Configure(EntityTypeBuilder<OrganizationMember> builder)
	{
		...
		builder.Property(x => x.Role).HasMaxLength(32).HasColumnName("role");

		builder.HasIndex(x => x.UserId).HasDatabaseName("organization_members_user_id_index");

		builder
			.HasIndex(x => new
			{
				x.OrganizationId, x.Role
			})
