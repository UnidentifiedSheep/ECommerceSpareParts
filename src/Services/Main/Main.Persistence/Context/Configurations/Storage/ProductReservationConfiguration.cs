...
public class ProductReservationConfiguration : IEntityTypeConfiguration<ProductReservation>
{
	public void Configure(EntityTypeBuilder<ProductReservation> builder)
	{
		...
		builder.HasIndex(
			e => new
			{
				e.ProductId, e.Status
			},
			"storage_content_reservations_product_id_status_index");

		builder
			.HasIndex(e => e.Comment, "storage_content_reservations_comment_index")
			.HasMethod("gin")
		...
		builder.Property(e => e.ProposedCurrencyId).HasColumnName("proposed_currency_id");

		builder.Property(e => e.ProposedPrice).HasColumnName("proposed_price");

		builder.Property(e => e.ReservedCount).HasColumnName("reserved_count");
		...
		builder
			.HasOne<Entities.Organization.Organization>(e => e.Organization)
			.WithMany()
