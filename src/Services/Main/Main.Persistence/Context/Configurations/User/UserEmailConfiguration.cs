...
public class UserEmailConfiguration : IEntityTypeConfiguration<UserEmail>
{
	public void Configure(EntityTypeBuilder<UserEmail> builder)
	{
		...
		builder.HasKey(e => e.Email).HasName("user_emails_primary_key");

		builder.HasIndex(e => e.UserId).HasDatabaseName("user_emails_user_id_index");
		...
		builder.Property(e => e.EmailType).HasMaxLength(50).HasColumnName("email_type");
		...
		builder
			.HasIndex(e => e.Email, "user_emails_normalized_email_index")
			.HasMethod("gin")
		...
		builder
			.HasOne<Entities.User.User>(e => e.User)
			.WithMany(p => p.Emails)
