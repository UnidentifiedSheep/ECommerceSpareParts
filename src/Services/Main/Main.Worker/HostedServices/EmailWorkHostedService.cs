...
public class EmailWorkHostedService(
	...
	ILogger<EmailWorkHostedService> logger) : BackgroundService
{
	...
	private async Task<bool> Iteration(EmailWorkOptions opt, CancellationToken ct)
	{
		try
		{
			...
			var result = await sender.Send(new SendMailBatchCommand(opt.ScheduleAtOnce), ct);
