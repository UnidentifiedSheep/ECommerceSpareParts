...
public class UserCacheRepository(
	...
	IProjectionProvider<User, UserDto> userProjection) : IUserCacheRepository
{
	public async Task<UserDto?> TryGetUserAsync(Guid userId, CancellationToken token = default)
	{
		...
		await rawCache.SetAsync([(key, user)], CacheKeys.UserCache.Ttl);
	...
	public async Task<decimal?> GetUserDiscountAsync(Guid userId, CancellationToken token = default)
	{
		...
		if (discount.HasValue)
			await rawCache.SetAsync([(key, discount)], CacheKeys.UserCache.Ttl);
		...
		CancellationToken token = default)
	{
		...
		await rawCache.SetAsync([(key, rolesAndPermissions)], CacheKeys.UserCache.Ttl);
	...
	public Task InvalidateUserDiscountAsync(Guid userId) =>
		rawCache.RemoveKeyAsync(CacheKeys.UserCache.UserDiscount(userId));

	public Task InvalidateUserRolesAndPermissionsAsync(Guid userId) =>
		rawCache.RemoveKeyAsync(CacheKeys.UserCache.UserRolesAndPermissions(userId));

	public Task InvalidateRolesAsync() =>
		rawCache.InvalidateByRelationsAsync(CacheKeys.UserCache.RolesAndPermissionsRelations());
	...
	private async Task<UserDto?> TryGetUserFromDb(Guid userId, CancellationToken token)
	{
		...
		return user == null ? null : userProjection.ProjectionFunc(user);
	}
}
