...
) : IProductProvider
{
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        if (uniqueRequests.Length == 0) return result;

        var cacheKeys = uniqueRequests
            .Select(GetCrossesCacheKey)
            .ToArray();
        ...
        if (missingRequests.Count == 0) return result;

        var loaded = await GetCrossesFromDb(
            missingRequests,
            cancellationToken);

        await rawCache.SetAsync(
            missingRequests.Select(request =>
                (GetCrossesCacheKey(request), loaded[request].ToArray())),
            CacheKeys.ProductCache.Ttl);
        ...
        var relations = missingRequests
            ...
            .ToDictionary(
                group => group.Key,
                group => group.Select(x => x.CacheKey).Distinct().ToList());
    ...
    {
        return CacheKeys.ProductCache.ProductCrosses(request.ProductId, request.SortBy);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var crosses = await crossesReadRepository.Query
            .GetCrosses(productIds)
            .ToListAsync(cancellationToken);
        var crossesByProductId = crosses.GetCrosses(productIds);

        var groups = requests.GroupBy(
            request => request.SortBy,
            SortByComparer.Instance);
        ...
        foreach (var group in groups)
        {
            ...
            if (crossProductIds.Count == 0) continue;

            var orderedCrossProductIds = await productReadRepository.Query
            ...
            foreach (var request in group)
            {
                ...
                result[request] = orderedCrossProductIds
                    .Where(requestCrosses.Contains)
                    .ToArray();
            }
        }
    ...
    private sealed class SortByComparer : IEqualityComparer<IReadOnlyList<string>>
    {
        ...
        public int GetHashCode(IReadOnlyList<string> value)
        {
            ...
            foreach (var item in value) hash.Add(item, StringComparer.Ordinal);
