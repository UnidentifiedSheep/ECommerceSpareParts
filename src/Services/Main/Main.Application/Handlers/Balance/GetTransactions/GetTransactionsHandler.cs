	...
	: IQueryHandler<GetTransactionsQuery, GetTransactionsResult>
{
	private static readonly List<TransactionStatus> ReversedStatuses =
		GetStatusesWithFlag(TransactionStatus.Reversed).ToList();
		...
		Guid? receiverId)
	{
		...
		if (senderId == receiverId)
		{
			...
			return query.Where(e => e.SenderId == organizationId || e.ReceiverId == organizationId);
		}
	...
	{
		var allFlagsMask = Enum
			.GetValues<TransactionStatus>()
			.Aggregate(0, (current, value) => current | (int)value);

		return Enumerable
			.Range(0, allFlagsMask + 1)
