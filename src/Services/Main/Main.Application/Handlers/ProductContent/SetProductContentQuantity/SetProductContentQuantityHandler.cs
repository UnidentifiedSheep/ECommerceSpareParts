...
[AutoSave]
[Transactional]
public record SetProductsContentCountCommand(int ParentProductId, int ChildProductId, int Quantity)
	: ICommand;
	...
	: ICommandHandler<SetProductsContentCountCommand>
{
	public async Task<Unit> Handle(
		SetProductsContentCountCommand request,
		CancellationToken cancellationToken)
	{
		var content =
			await contentRepository.GetById(
				(request.ParentProductId, request.ChildProductId),
				cancellationToken) ?? throw new ProductContentNotFoundException(
