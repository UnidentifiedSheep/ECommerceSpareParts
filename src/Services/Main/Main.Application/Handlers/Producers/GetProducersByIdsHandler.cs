...
{
	public GetProducersByIdsQuery(int id) : this([id])
	{
	}
...
public sealed record GetProducersByIdsResult(IReadOnlyDictionary<int, ProducerDto> Producers)
{
	...
	: IQueryHandler<GetProducersByIdsQuery, GetProducersByIdsResult>
{
	...
	{
		var producers = await repository
			.Query
			.Where(x => request.Ids.Contains(x.Id))
			.Project(projection)
			.ToDictionaryAsync(x => x.Id, cancellationToken);

		if (request.Ids.Count == 1 && !producers.ContainsKey(request.Ids[0]))
