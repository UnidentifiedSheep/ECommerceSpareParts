	...
	IUnitOfWork unitOfWork) : ICommandHandler<EditPurchaseCommand>
{
	...
	private Guid GetSystemUserId() => systemOptions.Value.SystemId;
	...
	private Task<Purchase> GetPurchase(Guid purchaseId, CancellationToken cancellationToken)
	{
		return purchaseRepository.EnsureExistForUpdateAsync(
			...
			Criteria<Purchase>.New().Include(x => x.Contents).Include(x => x.PurchaseLogistic),
			cancellationToken);
	}
	...
	{
		if (request is not { WithLogistics: true, StorageFrom: not null })
			return;
	...
	private async Task ReversePurchaseTransactions(Purchase purchase, CancellationToken cancellationToken)
	{
		await sender.Send(
		...
		CancellationToken cancellationToken)
	{
		...
		var requestedIds = contentDtos.Where(x => x.Id is not null).Select(x => x.Id!.Value).ToHashSet();

		requestedIds.EnsureAllExists(existingById.Keys, ids => new PurchaseContentNotFoundException(ids[0]));

		var storageContentIds = purchase.Contents.Select(x => x.StorageContentId).ToHashSet();
		...
		if (deletionSubtractions.Count > 0)
			await sender.Send(
				new SubtractStorageContentsCommand(deletionSubtractions, StorageMovementType.PurchaseEditing),
				cancellationToken);

		if (editingSubtractions.Count > 0)
			await sender.Send(
				new SubtractStorageContentsCommand(editingSubtractions, StorageMovementType.PurchaseEditing),
				cancellationToken);

		if (editingRestorations.Count > 0)
			await sender.Send(
				new RestoreContentCommand(editingRestorations, StorageMovementType.PurchaseEditing),
				cancellationToken);
	}
		...
		ICollection<SubtractStorageContentItem> subtractions)
	{
		...
		subtractions.Add(new SubtractStorageContentItem(content.StorageContentId, content.Count));
		...
		CancellationToken cancellationToken)
	{
		...
		for (var i = 0; i < result.StorageContents.Count; i++)
		{
			...
			if (storageContent.ProductId != newContent.ProductId || storageContent.Count != newContent.Count)
		...
		CancellationToken cancellationToken)
	{
		...
		if (countDelta < 0)
			subtractions.Add(new SubtractStorageContentItem(content.StorageContentId, -countDelta));
