...
public record ImportSupplierProductCommand(
	Supplier Supplier,
	IReadOnlyCollection<ContractSupplierProductDto> Products) : ICommand;

public class ImportSupplierProductHandler(
	ISupplierProductRepository repository,
	IUnitOfWork unitOfWork,
	ILogger<ImportSupplierProductHandler> logger) : ICommandHandler<ImportSupplierProductCommand>
{
	public async Task<Unit> Handle(ImportSupplierProductCommand request, CancellationToken cancellationToken)
	{
		...
		CancellationToken cancellationToken)
	{
		...
		return (await repository.GetBySupplierKeysAsync(
			...
			cancellationToken)).ToDictionary(x => new SupplierProductKey(x.Sku.NormalizedValue, x.Producer));
	}

	private ImportGraph BuildImportGraph(IEnumerable<ContractSupplierProductDto> rootProducts)
	{
		var products = new List<ImportProduct>();
		var crosses = new List<RequestedCross>();
		var processed =
			new Dictionary<ContractSupplierProductDto, ImportProduct>(ReferenceEqualityComparer.Instance);
		var visited = new HashSet<ContractSupplierProductDto>(ReferenceEqualityComparer.Instance);
		...
		[NotNullWhen(true)] out ImportProduct? importProduct)
	{
		...
		var key = new SupplierProductKey(Sku.ToNormalized(product.Number), product.Brand.Trim());
	...
	private record struct RequestedCross(SupplierProductKey Product, SupplierProductKey Analogue);

	private sealed record ImportProduct(SupplierProductKey Key, ContractSupplierProductDto Product);
