...
[AutoSave]
[Transactional]
public record UpdateOrganizationReservationCountsCommand(Guid OrganizationId, Dictionary<int, int> Contents)
...
public record UpdateOrganizationReservationCountsResult(Dictionary<int, int> NotFoundReservations);

public class UpdateOrganizationReservationCountsHandler(IRepository<ProductReservation, int> repository)
	: ICommandHandler<UpdateOrganizationReservationCountsCommand, UpdateOrganizationReservationCountsResult>
{
		...
		CancellationToken cancellationToken)
	{
		if (request.Contents.Count == 0)
			return new UpdateOrganizationReservationCountsResult([]);

		var organizationId = request.OrganizationId;
		var remaining = new Dictionary<int, int>(request.Contents);
		var productIds = remaining.Keys;

		var criteria = Criteria<ProductReservation>
			.New()
			.Where(x => x.OrganizationId == organizationId &&
				productIds.Contains(x.ProductId) &&
				(x.Status == ProductReservationStatus.Active || x.Status == ProductReservationStatus.Locked))
			.Track()
			.ForUpdate()
			.Build();
		...
		var notFoundReservations = remaining.Where(x => x.Value > 0).ToDictionary(x => x.Key, x => x.Value);
