    ...
    IQueryHandler<IsOrganizationSystemNameAvailableQuery, IsOrganizationSystemNameAvailableResult>
{
    ...
    {
        var any = await repository
            .Query
            .AnyAsync(x => 
                x.SystemName == Organization.NormalizeSystemName(request.SystemName), 
                cancellationToken);
