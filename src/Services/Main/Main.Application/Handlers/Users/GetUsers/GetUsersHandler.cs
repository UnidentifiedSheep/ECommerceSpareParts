...
public class GetUsersHandler(
	...
	IProjectionProvider<User, UserDto> projection) : IQueryHandler<GetUsersQuery, GetUsersResult>
{
		...
		CancellationToken cancellationToken)
	{
		...
		if (!string.IsNullOrWhiteSpace(request.Name))
		{
			...
			query = query.Where(u => EF.Functions.TrigramsSimilarity(
				u.UserInfo!.Name.ToUpper(CultureInfo.InvariantCulture),
				currName) > simLevel);
			isNameIncluded = true;
		}
		...
		if (!string.IsNullOrWhiteSpace(request.Surname))
		{
			...
			query = query.Where(u => EF.Functions.TrigramsSimilarity(
				u.UserInfo!.Surname.ToUpper(CultureInfo.InvariantCulture),
		...
		if (!string.IsNullOrWhiteSpace(request.Email))
		{
			...
			query = query.Where(u => u.Emails.Any(e =>
				EF.Functions.TrigramsSimilarity(e.Email.Value, normalizedEmail) > simLevel));
		...
		if (!string.IsNullOrWhiteSpace(request.Description))
		{
			...
			query = query.Where(u => u.UserInfo!.Description != null && EF.Functions.TrigramsSimilarity(
				u.UserInfo.Description.ToUpper(CultureInfo.InvariantCulture),
		...
		var queryWithScore = query
			.Select(u => new
			{
				User = u,
				Score = (isNameIncluded ? EF.Functions.TrigramsSimilarity(u.UserInfo!.Name, currName) : 0) +
					(isSurnameIncluded
		...
		return await queryWithScore.Select(x => x.User).Project(projection).ToListAsync(cancellationToken);
	}
	...
	private async Task<IReadOnlyList<UserDto>> GeneralSearch(
		...
		CancellationToken cancellationToken)
	{
		var query = readRepository.Query.Where(x => x.UserInfo != null).ExcludeUsersWithRole(Role.System);
		...
		return await query
			.Select(x => new
			{
				Rank = EF.Functions.TrigramsSimilarity(x.UserInfo!.SearchColumn, trimmed) +
					EF.Functions.TrigramsWordSimilarity(x.UserInfo!.SearchColumn, trimmed) * 0.7,
				User = x
			})
			.Where(x => x.Rank >= 0.3)
