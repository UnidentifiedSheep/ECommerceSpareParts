...
public record GetProductStorageContentsQuery(IReadOnlyCollection<GetProductStorageContentsItem> Items)
	...
	: IQueryHandler<GetProductStorageContentsQuery, GetProductStorageContentsResult>
{
		...
		CancellationToken cancellationToken)
	{
		...
		var resultIds = items.ToDictionary(item => item, _ => (IReadOnlyList<int>)[]);

		var groups = items.GroupBy(item => new
		{
			item.Pagination,
			item.StorageCode,
			item.ShowZeroCount
		...
		foreach (var group in groups)
		{
			...
			var contentsByProductId = contents
				...
				.ToDictionary(
					x => x.Key,
					IReadOnlyList<int> (x) => x.Select(content => content.StorageContentId).ToArray());
		...
		var contentIds = resultIds.Values.SelectMany(x => x).Distinct().ToList();
		...
		return new GetProductStorageContentsResult(
			resultIds.ToDictionary(
				item => item.Key,
				IReadOnlyList<StorageContentDto> (item) =>
					item.Value.Select(id => contentsById[id]).ToArray()));
	}
}
