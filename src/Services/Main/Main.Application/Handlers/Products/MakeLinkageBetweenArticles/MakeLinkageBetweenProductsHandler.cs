...
[AutoSave]
[Transactional]
public record MakeLinkageBetweenProductsCommand(IReadOnlyCollection<NewProductLinkageDto> Linkages)
	: ICommand<Unit>;

public class MakeLinkageBetweenProductsHandler(
	IProductRepository repository,
	IDomainEventScope domainEventScope) : ICommandHandler<MakeLinkageBetweenProductsCommand, Unit>
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		var criteria = Criteria<Product>.New().Where(x => productIds.Contains(x.Id)).Track(false).Build();
	...
	{
		var criteria = Criteria<Product>.New().Track(false).Build();
