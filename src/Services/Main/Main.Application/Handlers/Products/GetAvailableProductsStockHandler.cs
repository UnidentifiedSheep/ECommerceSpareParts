...
public record GetAvailableProductsStockQuery : IQuery<GetAvailableProductsStockResult>
{
    ...
    public GetAvailableProductsStockQuery(GetAvailableProductsStockItem item)
        : this([item]) { }
}
...
) : IQueryHandler<GetAvailableProductsStockQuery, GetAvailableProductsStockResult>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var productIds = request.Items
            ...
            .Distinct()
            .ToList();
        var storageCodes = request.Items
            .Select(x => x.StorageCode)
            .Distinct()
            .ToList();
        ...
        var result = (await storageContentReadRepository
                ...
                .Where(x => 
                    ...
                    storageCodes.Contains(x.StorageCode))
                .GroupBy(x => new
                {
                    x.ProductId,
                    x.StorageCode
                ...
                .Where(x => requestedItems.Contains(
                    new GetAvailableProductsStockItem(
                        x.ProductId,
                        x.StorageCode)))
                .ToDictionary(
