...
public class GetRoleHandler(
	...
	ISender sender,
	IProjectionProvider<Role, RoleDto> projection) : IQueryHandler<GetRoleQuery, GetRoleResult>
{
	public async Task<GetRoleResult> Handle(GetRoleQuery request, CancellationToken cancellationToken)
	{
		...
		var permissions = (await sender.Send(new GetPermissionsQuery(), cancellationToken)).Permissions;

		var roleWithPermissions = await readRepository
			.Query
			.Where(x => x.Name == request.Name)
			.AsExpandable()
			.Select(x => new
			{
				Role = roleToDto.Invoke(x), Permissions = x.RolePermissions.Select(z => z.PermissionName)
			})
			.FirstOrDefaultAsync(cancellationToken) ?? throw new RoleNotFoundException(request.Name);
