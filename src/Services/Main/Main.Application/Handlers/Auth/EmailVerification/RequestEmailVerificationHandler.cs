    ...
    : ICommandHandler<RequestEmailVerificationCommand>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var criteria = Criteria<UserEmail>.New()
            .Where(x => x.Email == normalizedEmail)
            .Track(false)
            .Build();
        var userMail = await repository
            .FirstOrDefaultAsync(
                criteria, 
                cancellationToken);
        ...
        if (userMail.Confirmed) return Unit.Value;

        var setting = (await settingsService
                .GetOrDefault<GlobalApplicationSetting>(cancellationToken)).Data;
        ...
        var verificationUrl = new Uri(
            baseUri,
            $"verify-email?token={Uri.EscapeDataString(signed)}");
