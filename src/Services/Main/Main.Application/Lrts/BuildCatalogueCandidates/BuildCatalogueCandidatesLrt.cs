...
public class BuildCatalogueCandidatesLrt(
	...
	ILogger<BuildCatalogueCandidatesLrt> logger) : LrtBase<NoneInputState, BuildCatalogueCandidatesState>(
	jobRepository,
	unitOfWork,
	publisher,
	transactionService,
	logger)
{
	...
	private static readonly TransactionalAttribute BatchTransactionSettings = new(
	...
	private Task<BatchResult> ProcessBatchAsync(int lastProcessedId, int batchSize)
	{
		...
		return TransactionService.ExecuteAsync(
			...
			async (context, cancellationToken) =>
			{
				...
				var candidateGroups = resolvedProducts
					.GroupBy(x => (x.Product.Sku.NormalizedValue, ProducerId: x.ProducerId!.Value))
					.ToDictionary(x => x.Key, x => x.ToList());

				var normalizedSkus = candidateGroups.Keys.Select(x => x.NormalizedValue).Distinct().ToList();
				var producerIds = candidateGroups.Keys.Select(x => x.ProducerId).Distinct().ToList();
				...
				var persistedCandidates = (await catalogueCandidateRepository.ListAsync(
						Criteria<CatalogueCandidate>
							...
							.Build(),
						cancellationToken))
					.Where(x => candidateGroups.ContainsKey((x.Sku.NormalizedValue, x.ProducerId)))
					.ToDictionary(x => (x.Sku.NormalizedValue, x.ProducerId));

				var missingCandidates = candidateGroups
					.Where(x => !persistedCandidates.ContainsKey(x.Key))
					.Select(x => CatalogueCandidate.Create(x.Value[0].Product.Sku.Value, x.Key.ProducerId))
					.ToList();

				if (missingCandidates.Count > 0)
				{
					await context.UnitOfWork.AddRangeAsync(missingCandidates, cancellationToken);
				...
				{
					var key = (resolved.Product.Sku.NormalizedValue, resolved.ProducerId!.Value);
					persistedCandidates[key].AddSupplierProduct(resolved.Product);
				}
	...
	private sealed record ResolvedSupplierProduct(SupplierProduct Product, int? ProducerId);
