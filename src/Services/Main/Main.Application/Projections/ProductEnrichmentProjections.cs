...
public sealed class
	SupplierProductNameDtoProjectionProvider : ProjectionProviderBase<SupplierProductName,
	SupplierProductNameDto>
{
	public override Expression<Func<SupplierProductName, SupplierProductNameDto>> Projection { get; } = x =>
		new SupplierProductNameDto
		{
			Id = x.Id,
...
[Lifetime(Lifetime.Singleton)]
public sealed class
	SupplierProductDtoProjectionProvider : ProjectionProviderBase<SupplierProduct, SupplierProductDto>
{
...
[Lifetime(Lifetime.Singleton)]
public sealed class
	CatalogueCandidateReviewDtoProjectionProvider : ProjectionProviderBase<CatalogueCandidate,
	CatalogueCandidateReviewDto>
{
		...
		IProjectionProvider<SupplierProduct, SupplierProductDto> supplierProductProjection)
	{
		...
		Projection = x => new CatalogueCandidateReviewDto
		{
			...
			Product = x.Product == null ? null : productToDto.Invoke(x.Product),
			Sku = x.Sku.Value,
			SupplierProducts = x
				.SupplierProducts
				.OrderBy(z => z.Id)
...
[Lifetime(Lifetime.Singleton)]
public sealed class
	CatalogueCandidateContractDtoProjectionProvider : ProjectionProviderBase<CatalogueCandidate,
	CatalogueCandidateContractDto>
{
	public CatalogueCandidateContractDtoProjectionProvider()
	{
		Projection = x => new CatalogueCandidateContractDto
		{
			...
			Names = x.SupplierProducts.SelectMany(z => z.Names.Select(c => c.Name.Trim())).ToList()
		};
	}
