...
[Lifetime(Lifetime.Singleton)]
public sealed class FullProductDtoProjectionProvider : ProjectionProviderBase<Product, FullProductDto>
{
		...
		IProjectionProvider<ProductSize, ProductSizeDto> productSizeProjection)
	{
		...
		Projection = x => new FullProductDto
		{
			...
			ProductSize = x.ProductSize == null ? null : sizeToDto.Invoke(x.ProductSize)
		};
	}
...
{
	public override Expression<Func<ProductWeight, ProductWeightDto>> Projection { get; } = x =>
...
[Lifetime(Lifetime.Singleton)]
public sealed class ProductSizeDtoProjectionProvider : ProjectionProviderBase<ProductSize, ProductSizeDto>
{
	public override Expression<Func<ProductSize, ProductSizeDto>> Projection { get; } = x =>
		new ProductSizeDto
		{
			ProductId = x.ProductId,
			Unit = x.Unit,
