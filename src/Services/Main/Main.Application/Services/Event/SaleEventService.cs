...
{
	public static readonly Expression<Func<Sale, SaleEventModel>> ToSaleEvent = x => new SaleEventModel
	{
		Id = x.Id,
		...
		State = x.State == SaleState.Completed ? SaleStateEventModel.Completed :
			x.State == SaleState.Deleted ? SaleStateEventModel.Deleted : SaleStateEventModel.Draft,
	...
	private static readonly Expression<Func<SaleContent, SaleContentEventModel>> ToSaleContentEvent = x =>
		new SaleContentEventModel
		{
			Comment = x.Comment,
			Count = x.Count,
			Discount = x.Discount,
	...
	private static readonly Expression<Func<SaleContentDetail, SaleContentDetailEventModel>>
		ToSaleContentDetailEvent = x => new SaleContentDetailEventModel
		{
			BuyPrice = x.BuyPrice,
	...
	public async Task NotifyUpdated(Guid id, CancellationToken cancellationToken)
	{
		var baseCurrencyId = (await settingsService.GetOrDefault<CurrencySetting>(cancellationToken)).Data
			.BaseCurrencyId;

		var fromDb = await repository
			.Query
			.Where(x => x.Id == id)
			.AsExpandable()
			.Select(ToSaleEvent)
			.FirstOrDefaultAsync(cancellationToken) ?? throw new SaleNotFoundException(id);
