...
public class PurchaseLogisticsService(ISender sender, IUserRepository userRepository, IUnitOfWork unitOfWork)
	: IPurchaseLogisticsService
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		foreach (var content in purchase
					.Contents
					.Except(selectedItems.Select(x => x.PurchaseContent))
					.ToList())
