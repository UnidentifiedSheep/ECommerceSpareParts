...
public class SaleService(ISender sender, IProductRepository productRepository) : ISaleService
{
	...
	public async Task UpdateOrganizationReservationCounts(
		...
		CancellationToken cancellationToken = default)
	{
		var toSubtract = counts.Where(x => x.Value > 0).ToDictionary(x => x.Key, x => x.Value);

		if (toSubtract.Count == 0)
			return;
		...
		CancellationToken cancellationToken = default)
	{
		var toRestore = sale
			.Contents
			.SelectMany(content =>
				content.Details.Select(detail => new RestoreContentItem(detail, content.ProductId)))
			.ToList();
		...
		IEnumerable<SaleContentInput> saleContents)
	{
		...
		return result.OrderBy(x => x.OriginalIndex).Select(x => x.Content).ToList();
	}
		...
		CancellationToken cancellationToken)
	{
		return await sender.Send(
			new SubtractStorageContentsCommand(
				contents.Select(x => new SubtractProductFromStorageItem(
					x.ProductId,
					storageCode,
					x.Count,
					takeFromOtherStorages)),
		...
		CancellationToken cancellationToken)
	{
		...
		var products =
			(await productRepository.ListAsync(criteria, cancellationToken)).ToDictionary(x => x.Id);
		...
		foreach (var (id, count) in byReservation.OrderBy(x => x.Key))
		{
			...
			res[key] = count;
			codeBuilder.Append(CultureInfo.InvariantCulture, $"{key}:{count}");
		}
