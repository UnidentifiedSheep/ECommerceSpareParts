...
public class PerAreaPricing : LogisticsPricingStrategyBase
{
    ...
    public override LogisticsCalcResult Calculate(LogisticsContext context, IEnumerable<LogisticsItem> items)
    {
        return Iterate(
            context,
            items,
            input => CalculatePrice(input.AreaM3, context),
            LogisticsDataRequirements.Size);
    }

    private decimal CalculatePrice(decimal area, LogisticsContext context) { return context.PriceM3 * area; }
}