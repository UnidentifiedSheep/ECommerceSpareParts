...
public abstract class LogisticsPricingStrategyBase : ILogisticsPricingStrategy
{
		...
		LogisticsDataRequirements requirements)
	{
		...
		{
			PricingModel = Type, MinimalPrice = context.MinimumPrice ?? 0
		};
		...
		{
			if (item.Quantity <= 0)
				throw new ArgumentException("Количество должно быть больше или равно 0.");
			...
			if (cost < 0)
				throw new ArgumentException("Цена должна быть больше или равна 0");
	...
	{
		if (weight < 0)
			throw new ArgumentException("Вес должно быть больше или равно 0.");
		if (area < 0)
			throw new ArgumentException("Площадь должно быть больше или равно 0.");
		...
		return reasons.Count > 0 ? (true, reasons) : (false, null);
	}
}
