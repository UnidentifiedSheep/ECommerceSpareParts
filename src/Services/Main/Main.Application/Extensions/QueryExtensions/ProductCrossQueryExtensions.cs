...
{
	public static IQueryable<Product> GetCrosses(this IQueryable<ProductCross> query, int productId)
	{
		var left = query.Where(c => c.RightProductId == productId).Select(c => c.LeftProduct);

		var right = query.Where(c => c.LeftProductId == productId).Select(c => c.RightProduct);
		...
		IReadOnlySet<int> productIds)
	{
		return query.Where(x =>
			productIds.Contains(x.LeftProductId) || productIds.Contains(x.RightProductId));
	}
	...
	public static IReadOnlyDictionary<int, IReadOnlySet<int>> GetCrosses(
		...
		IReadOnlySet<int> productIds)
	{
		var result = productIds.ToDictionary(productId => productId, _ => new HashSet<int>());
		...
		return result.ToDictionary(item => item.Key, IReadOnlySet<int> (item) => item.Value);
	}
}
