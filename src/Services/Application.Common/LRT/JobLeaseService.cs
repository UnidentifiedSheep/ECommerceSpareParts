...
public class JobLeaseService(IApplicationTransactionService transactionService) : IJobLeaseService
{
	public async Task<Job?> TryAcquireJobAsync(
		Guid holderId,
		TimeSpan leaseDuration,
		CancellationToken ct) => await transactionService.ExecuteAsync(
		...
		async (context, cancellationToken) =>
		{
			...
			var criteria = GetCriteriaBase(1)
				.Where(x =>
					...
					x.LeaseExpiresAt != null && x.LeaseExpiresAt <= now && x.Attempts < x.MaxAttempts)
				.Build();
	...
	private static CriteriaBuilder<Job> GetCriteriaBase(int maxBatchSize) => Criteria<Job>
		.New()
