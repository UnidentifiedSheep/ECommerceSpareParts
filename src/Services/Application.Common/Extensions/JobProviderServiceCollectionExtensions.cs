...
{
    public static IServiceCollection RegisterJobProviders<TAssemblyMarker>(
        this IServiceCollection services)
    {
        return services.RegisterJobProviders(typeof(TAssemblyMarker).Assembly);
    }
                     ...
                     .Where(x => x is { IsClass: true, IsAbstract: false }))
        {
            var servicesToRegister = implementation.ImplementedInterfaces
                .Where(x => x.IsGenericType &&
                            x.GetGenericTypeDefinition() == jobProviderType)
                .ToList();
            ...
            var lifetime = implementation.GetCustomAttribute<LifetimeAttribute>()?.Lifetime
                           ?? throw new InvalidOperationException(
                               $"Job provider {implementation.FullName} must have " +
