...
public static class RepositoryExtensions
{
	public static Task<Dictionary<TKey, TEntity>> EnsureExistsAsync<TEntity, TKey>(
		...
		CancellationToken ct = default) where TEntity : Entity<TEntity, TKey> where TKey : notnull
	{
		var criteria = Criteria<TEntity>.New().Track().Build();
	...
	public static Task<Dictionary<TKey, TEntity>> EnsureExistsForUpdateAsync<TEntity, TKey>(
		...
		CancellationToken ct = default) where TEntity : Entity<TEntity, TKey> where TKey : notnull
	{
		var criteria = Criteria<TEntity>.New().Track().ForUpdate().Build();
		...
		CancellationToken ct = default) where TEntity : Entity<TEntity, TKey> where TKey : notnull
	{
		...
		keySet.EnsureAllExists(result.Keys, errorFactory);
	...
	public static async Task<TEntity> EnsureExistForUpdateAsync<TEntity, TKey>(
		...
		where TEntity : Entity<TEntity, TKey>, ILinqEntity<TEntity, TKey> where TKey : notnull
	{
	...
	private static async Task<TEntity> EnsureExistCoreAsync<TEntity, TKey>(
		...
		where TEntity : Entity<TEntity, TKey>, ILinqEntity<TEntity, TKey> where TKey : notnull
	{
		var criteria = criteriaBuilder.Where(TEntity.GetEqualityExpression(key)).Build();

		var result = await repository.FirstOrDefaultAsync(criteria, ct);
