...
public class DomainEventScope(IPublisher publisher) : IDomainEventScope
{
	...
	public Task PublishImmediatelyAsync<T>(T @event, CancellationToken ct = default) where T : IDomainEvent =>
	...
	{
		var result = new List<IDomainEvent>(_events.Count + _keyedEvents.Count + _buckets.Count);
	...
	{
		var factory = BucketFactories.GetOrAdd(
			eventType,
			static type =>
			{
