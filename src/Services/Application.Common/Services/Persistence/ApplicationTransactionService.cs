	...
	IApplicationTransactionContext context) : IApplicationTransactionService
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		Task<TResult> ExecuteCoreAsync() => ExecuteBoundaryAsync(action, cancellationToken);
	}
		...
		try
		{
			...
			foreach (var @event in integrationEvents)
				if (@event.RoutingKey is null)
					await publisher.Publish(@event.Message, cancellationToken);
				else
			...
			if (integrationEvents.Count != 0 && !unitOfWork.Context.SuppressAutoSave)
