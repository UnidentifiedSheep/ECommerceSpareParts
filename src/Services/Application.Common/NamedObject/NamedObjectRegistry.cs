	...
	where TBaseObject : class, INamedObject
{
	...
	{
		_objects = objects.ToDictionary(x => x.SystemName, StringComparer.OrdinalIgnoreCase);
	}
	...
	INamedObject INamedObjectRegistry.GetBySystemName(string systemName) => GetBySystemName(systemName);
}
