	...
	IOptionsMonitor<ScheduledJobEnqueuerOptions> options) : BackgroundService
{
	...
	private async Task<bool> Iteration(ScheduledJobEnqueuerOptions opt, CancellationToken ct)
	{
		try
		{
			await using var scope = scopeFactory.CreateAsyncScope();
			var jobScheduleService = scope.ServiceProvider.GetRequiredService<IJobScheduleService>();

			var queued = await jobScheduleService.QueueDueSchedulesAsync(opt.BatchSize, ct);
