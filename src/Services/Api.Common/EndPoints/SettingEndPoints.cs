...
public class SettingEndPoints : ICarterModule
{
	...
	{
		var settings = app.MapGroup("/settings").WithTags("Settings");
		...
		settings
			.MapPut(
				"{settingName}",
				async (
					ISender sender, string settingName,
					UpdateSettingRequest request, CancellationToken ct) =>
				{
					await sender.Send(new UpdateSettingCommand(settingName, request.Json), ct);
