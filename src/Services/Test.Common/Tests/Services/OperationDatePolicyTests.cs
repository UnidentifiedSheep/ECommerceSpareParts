...
public class OperationDatePolicyTests
{
    ...
    {
        var permission = nameof(PermissionCodes.CREATE_HISTORICAL_RECORDS)
            .ToNormalizedPermission();
        var policy = CreatePolicy(new HashSet<string> { permission });
    ...
    private static OperationDatePolicy CreatePolicy(IReadOnlySet<string>? permissions = null)
    {
        ...
        timeProvider
            .Setup(x => x.GetUtcNow())
            .Returns(new DateTimeOffset(Now));

        var userContext = new Mock<IUserContext>();
        userContext
            .SetupGet(x => x.Permissions)
            .Returns(permissions ?? new HashSet<string>());
