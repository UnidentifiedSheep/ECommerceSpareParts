...
public sealed class SettingsServiceTests
{
	...
	public async Task SetSetting_ExistingSetting_UsesApplicationTransaction()
	{
		...
		repository
			.Setup(x => x.FirstOrDefaultAsync(It.IsAny<Criteria<Setting>>(), It.IsAny<CancellationToken>()))
		...
		transactionContext.SetupGet(x => x.UnitOfWork).Returns(unitOfWork.Object);
		...
		transactionService
			...
			.Returns<TransactionalAttribute?, Func<IApplicationTransactionContext, CancellationToken, Task>,
				CancellationToken>((
				_, action,
				ct) => action(transactionContext.Object, ct));
	...
	private sealed class TestSetting(TestSettingData data) : Setting<TestSettingData>(Name, data)
	{
