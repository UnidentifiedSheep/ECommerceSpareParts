...
public class MultiStepJobTests
{
	...
	public void CancelUnfinishedSteps_CancelsEveryNonTerminalStep()
	{
		...
		processing.AcquireLease(processingLeaseHolderId, TimeSpan.FromMinutes(5));
		processing.Start(processingLeaseHolderId);

		var succeededLeaseHolderId = Guid.NewGuid();
		succeeded.AcquireLease(succeededLeaseHolderId, TimeSpan.FromMinutes(5));
		succeeded.Start(succeededLeaseHolderId);
		succeeded.Succeed(succeededLeaseHolderId);

		job.CancelUnfinishedSteps(job.Steps, "workflow failed");
	...
	public void AcquireLease_BlockedStep_Throws()
	{
		...
		var act = () => step.AcquireLease(Guid.NewGuid(), TimeSpan.FromMinutes(5));
	...
	public void AddStep_ActivatedNestedMultiStepJob_Throws()
	{
		...
		var act = () => nested.AddStep(SingleRunJob.Create("late-step", "{}"));
	...
	public void NonTerminalStepUpdate_DoesNotRaiseFinishedEvent()
	{
		...
		step.FlushDomainEvents().OfType<JobStepFinishedDomainEvent>().Should().BeEmpty();
	}

	private static MultiStepJob Create() => MultiStepJob.Create("multi-step", "{}");
