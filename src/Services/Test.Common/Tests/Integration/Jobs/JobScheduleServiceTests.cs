    ...
    : CommonLayerIntegrationTest(fixture)
{
    ...
    private IJobScheduleService Service => Scope.ServiceProvider
    ...
    public override async Task InitializeAsync()
    {
        ...
        Scope.ServiceProvider
            .GetRequiredService<TestTimeProvider>()
            .SetUtcNow(UtcNow);
    }
    ...
    public async Task Create_ValidSchedule_PersistsAndCalculatesNextRun()
    {
        ...
        schedule.NextRunAt.Should().Be(
            new DateTime(2026, 8, 21, 10, 5, 0, DateTimeKind.Utc));
    }

    [Fact]
    public async Task Update_CronChanged_PersistsAndRecalculatesNextRun()
    {
        var schedule = await AddScheduleAsync(enabled: true);
    ...
    {
        var action = () => Service.UpdateScheduleAsync(
            Guid.NewGuid(),
            new PatchJobScheduleDto());
