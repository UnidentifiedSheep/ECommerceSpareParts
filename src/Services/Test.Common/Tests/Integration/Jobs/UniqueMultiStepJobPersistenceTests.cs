...
public sealed class UniqueMultiStepJobPersistenceTests(CombinedContainerFixture fixture)
	: CommonLayerIntegrationTest(fixture)
{
	...
	public async Task TryAddAsync_DuplicateMultiStepJob_SkipsSecondGraph()
	{
		...
		(await Context.Jobs.AnyAsync(x => x.Id == duplicate.Id, CancellationToken)).Should().BeFalse();
	}
	...
	public async Task TryAddAsync_ConcurrentDuplicates_AddsOnlyOneGraph()
	{
		...
		var results = await Task.WhenAll(
			AddJobsAsync(
				...
				[CreateUniqueWorkflow("workflow:42"), CreateUniqueWorkflow("workflow:43")]),
			AddJobsAsync(
				secondScope.ServiceProvider,
				[CreateUniqueWorkflow("workflow:43"), CreateUniqueWorkflow("workflow:42")]));

		results.Should().BeEquivalentTo([0, 2]);

		Context.ChangeTracker.Clear();
		var rootCount = await Context
			.Jobs
			.AsNoTracking()
			.CountAsync(
				x => x.MultiStepJobId == null && x.SystemName == "workflow" &&
					(x.NaturalKey == "workflow:42" || x.NaturalKey == "workflow:43"),
				CancellationToken);
		var stepCount = await Context
			.Jobs
			.AsNoTracking()
			.CountAsync(x => x.MultiStepJobId != null, CancellationToken);
	...
	public async Task TryAddAsync_MixedBatch_PersistsSingleRunAndMultiStepJobs()
	{
		...
		Context.ChangeTracker.Clear();
		(await Context.Jobs.AnyAsync(x => x.Id == singleRunJob.Id, CancellationToken)).Should().BeTrue();
		var persistedWorkflow = await Context
			.Jobs
			.AsNoTracking()
			.OfType<MultiStepJob>()
			.Include(x => x.Steps)
			.SingleAsync(x => x.Id == multiStepJob.Id, CancellationToken);
	...
	public async Task TryAddAsync_PartialSingleRunConflict_ReturnsOnlyInsertedId()
	{
		...
		addedIds.Should().ContainSingle().Which.Should().Be(newJob.Id);
	}
	...
	public async Task InsertJobsAsync_NonUniqueBatch_PersistsEveryJob()
	{
		...
		addedIds.Should().BeEquivalentTo([first.Id, second.Id, workflow.Id]);

		Context.ChangeTracker.Clear();
		var persistedIds = await Context
			.Jobs
			.AsNoTracking()
	...
	private Task<int> AddJobsAsync(Job job) => AddJobsAsync([job]);
	...
	private Task<IReadOnlyList<Guid>> AddJobIdsAsync(IEnumerable<Job> jobs) =>
		AddJobIdsAsync(Scope.ServiceProvider, jobs);

	private static async Task<int> AddJobsAsync(IServiceProvider serviceProvider, Job job) =>
		await AddJobsAsync(serviceProvider, [job]);

	private static async Task<int> AddJobsAsync(IServiceProvider serviceProvider, IEnumerable<Job> jobs)
	{
	...
	{
		var transactionService = serviceProvider.GetRequiredService<IApplicationTransactionService>();
		...
		return await transactionService.ExecuteAsync(
			...
			async (context, cancellationToken) =>
			{
				...
				var addedIds = await repository.InsertJobsAsync(jobs, cancellationToken);
