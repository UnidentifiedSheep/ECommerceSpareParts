...
public class LocalizationTests
{
	public async Task TestLocalizableExceptions(
		...
		Locale locale)
	{
		var scoped = await CreateLocalizer(path, locale);
		CultureInfo.CurrentCulture = CultureInfo.CurrentUICulture = new CultureInfo(locale);

		var exceptionTypes = exceptionsAssembly.DefinedTypes.Where(t =>
			typeof(ILocalizableException).IsAssignableFrom(t) &&
		...
		Locale locale)
	{
		...
		var validatorTypes = validatorsAssembly
			.DefinedTypes
		...
		foreach (var type in validatorTypes)
		{
			...
			var membersAndValidators = descriptor.GetMembersWithValidators().SelectMany(x => x.ToList());

			foreach (var (_, o) in membersAndValidators)
			{
	...
	private static IValidator CreateValidator(Type type)
	{
		...
		var args = ctor.GetParameters().Select(p => GetDefault(p.ParameterType)).ToArray();

		return (IValidator)Activator.CreateInstance(type, args)!;
	}

	public async Task<IContextualStringLocalizer> CreateLocalizer(string path, Locale locale)
	{
	...
	private static object? GetDefault(Type type)
	{
		...
		if (type.IsGenericType)
		{
			...
			if (typeof(IEnumerable<>).IsAssignableFrom(genericType) ||
				...
				typeof(IList<>).IsAssignableFrom(genericType) || genericType == typeof(List<>))
			{
