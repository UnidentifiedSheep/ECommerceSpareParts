...
public sealed class ResumeMultiStepJobHandlerTests
{
    ...
    public async Task Handle_MultipleParents_ResumesWaitingParentsWithOneQuery()
    {
        ...
        [
            CreateEvent(firstParent.Id),
            CreateEvent(firstParent.Id),
            CreateEvent(secondParent.Id)
        ]);
    ...
    private static JobStepFinishedDomainEvent CreateEvent(Guid parentId) =>
        new(Guid.NewGuid(), parentId, JobStatus.Succeeded);
