...
public class LrtBaseTests
{
	...
	private sealed class TestLrt(
		...
		ILogger logger) : LrtBase<TestInput, TestState>(
		jobRepository,
		unitOfWork,
		publisher,
		transactionService,
		logger)
	{
		...
		public void InterruptForTest(string reason) => Interrupt(reason);

		public void RequestCancellationForTest(string reason) => Job.RequestCancellation(reason);
		...
		public Task UpdateStateForTest(TestState state) => SaveStateAsync(state);

		public Task RenewLeaseForTest(TimeSpan leaseDuration) => RenewLeaseAsync(leaseDuration);
	}
