...
public class LrtQuotaManagerTests
{
	...
	public async Task UseQuotaAsync_WhenNoQuotaAvailable_WaitsUntilReleased()
	{
		...
		var waitingTask =
			manager.UseQuotaAsync(waitingHolderId, TestContext.Current.CancellationToken).AsTask();

		waitingTask.IsCompleted.Should().BeFalse();
		acquiredQuota.Dispose();

		using var quota = await waitingTask.WaitAsync(
	...
	public async Task TryUseQuota_ConcurrentCalls_AcquiresNoMoreThanMaxQuota()
	{
		...
		var holderIds = Enumerable.Range(0, callsCount).Select(_ => Guid.NewGuid()).ToArray();

		var results = await Task.WhenAll(
			holderIds.Select(holderId => Task.Run(() =>
			{
				var acquired = manager.TryUseQuota(holderId, out var quota);
				return new
				{
					Acquired = acquired, Quota = quota
				};
			})));
		...
		var results = await Task.WhenAll(
			holderIds.Select(holderId => Task.Run(() =>
			{
				try
				{
					return new
					{
						Quota = (ILrtQuota?)manager.UseQuota(holderId), Exception = (Exception?)null
					};
				}
				catch (Exception ex)
				{
					return new
					{
						Quota = (ILrtQuota?)null, Exception = (Exception?)ex
					};
				}
			})));
