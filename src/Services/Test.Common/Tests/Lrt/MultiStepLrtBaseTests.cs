...
public sealed class MultiStepLrtBaseTests
{
	...
	private sealed class Fixture
	{
		...
		public Fixture()
		{
			...
			registry
				.Setup(x => x.GetBySystemName(
					It.Is<string>(name => name == "first-root" || name == "second-root" || name == "child")))
				.Returns((string name) => CreateSingleRunLrt(name));
			Parent = (MultiStepJob)new JobCreationDispatcher(registry.Object).Create(
				Lrt.SystemName,
				"{}",
				3);
			...
			JobRepository
				.Setup(x => x.FirstOrDefaultAsync(It.IsAny<Criteria<Job>>(), It.IsAny<CancellationToken>()))
				.ReturnsAsync(Parent);
			JobRepository
				.Setup(x => x.ListAsync(It.IsAny<Criteria<Job>>(), It.IsAny<CancellationToken>()))
				.ReturnsAsync(() => Parent.Steps.ToList());
		}
		...
		public Task ExecuteAsync() => Lrt.ExecuteAsync(Parent.Id, _parentLeaseHolderId);
