...
internal sealed class PendingUniqueJobFilter<TContext>(TContext context) where TContext : DbContext
{
	public async Task<IReadOnlyList<Job>> FilterAsync(
		IReadOnlyCollection<Job> jobs,
		CancellationToken cancellationToken)
	{
		var uniqueJobs = jobs.Where(x => x.NaturalKey is not null).ToList();

		if (uniqueJobs.Count == 0)
			return jobs.ToList();

		var existingKeys = await LockAndGetExistingKeysAsync(uniqueJobs, cancellationToken);

		return jobs
			.Where(x =>
				x.NaturalKey is null || !existingKeys.Contains(new JobKey(x.SystemName, x.NaturalKey)))
			.ToList();
	}
	...
	private sealed record JobKey(string SystemName, string NaturalKey);
}
