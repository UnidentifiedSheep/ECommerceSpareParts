...
internal sealed class JobRepository<TContext>(
	...
	: LinqRepositoryBase<TContext, Job, Guid>(context, extensions), IJobRepository where TContext : DbContext
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		if (Context.Database.CurrentTransaction is null)
			throw new InvalidOperationException("Jobs must be inserted inside a database transaction.");

		var insertable = await uniqueJobFilter.FilterAsync(all, cancellationToken);
	...
	private static List<Job> Normalize(IEnumerable<Job> jobs)
	{
		...
		foreach (var job in jobs)
		{
			...
			if (job.NaturalKey is not null && !uniqueKeys.Add(new JobKey(job.SystemName, job.NaturalKey)))
				continue;
	...
	private static void EnsureValid(IReadOnlyCollection<Job> jobs)
	{
		if (jobs.Any(x => x is not SingleRunJob and not MultiStepJob))
			throw new InvalidOperationException("Unsupported job type in job batch.");
	}

	private readonly record struct JobKey(string SystemName, string NaturalKey);
}
