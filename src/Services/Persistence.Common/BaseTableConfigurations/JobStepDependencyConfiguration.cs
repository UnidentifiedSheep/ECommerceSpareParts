...
public sealed class JobStepDependencyConfiguration : IEntityTypeConfiguration<JobStepDependency>
{
	public void Configure(EntityTypeBuilder<JobStepDependency> builder)
	{
		...
		builder
			.HasKey(x => new
			{
				x.StepId, x.DependsOnStepId
			})
			.HasName("job_step_dependencies_pk");

		builder.Property(x => x.StepId).HasColumnName("step_id");

		builder.Property(x => x.MultiStepJobId).HasColumnName("multi_step_job_id");

		builder.Property(x => x.DependsOnStepId).HasColumnName("depends_on_step_id");
