...
public static class ModelBuilderExtensions
{
	...
	public static ModelBuilder AddFieldsForAuditableEntities(this ModelBuilder modelBuilder)
	{
		foreach (var entityType in modelBuilder.Model.GetEntityTypes())
		{
			...
			builder.Property(nameof(IAuditable.CreatedAt)).HasColumnName("created_at");

			builder.Property(nameof(IAuditable.UpdatedAt)).HasColumnName("updated_at");

			builder.Property(nameof(IAuditable.WhoCreated)).HasColumnName("who_created");

			builder.Property(nameof(IAuditable.WhoUpdated)).HasColumnName("who_updated");

			builder
				.HasIndex(nameof(IAuditable.WhoUpdated))
				.HasDatabaseName($"{entityType.Name.ToLowerInvariant()}_who_updated_idx");

			builder
				.HasIndex(nameof(IAuditable.WhoCreated))
