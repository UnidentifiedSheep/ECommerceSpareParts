...
{
	public SalesProfitOverTimeChartDataSourceTests(CombinedContainerFixture fixture) : base(fixture)
	{
	...
	public async Task QueryAsync_AggregatesMetricsRoundsDecimalsAndExcludesDeletedFacts()
	{
		...
		{
			TestContext.SalesFact, TestContext.SameDaySalesFact
		};

		var result = await QueryAsync(StartOfDay(TestContext.Period), EndOfDay(TestContext.Period));
		...
		dataPoint.Revenue.Should().Be(RoundedSum(includedFacts, fact => fact.RevenueInBaseCurrency));
		dataPoint.Cost.Should().Be(RoundedSum(includedFacts, fact => fact.CostInBaseCurrency));
		dataPoint.GrossProfit.Should().Be(RoundedSum(includedFacts, fact => fact.GrossProfitInBaseCurrency));
		...
		int expectedDataPoints)
	{
		...
		result.DataPoints.Sum(x => x.SalesCount).Should().Be(TestContext.ActiveSalesFacts.Count);
	}
	...
	{
		var result = await QueryAsync(TestContext.SalesFact.CreatedAt, TestContext.SalesFact.CreatedAt);
	...
	public async Task QueryAsync_AppliesCursorPagination()
	{
		...
		secondPage
			...
			.PeriodStart
			.Should()
			.Be(StartOfDay(TestContext.SameMonthSalesFact.CreatedAt));
	}
	...
	private Task<ChartDataResult<SalesProfitDataPoint>> QueryAsync(SalesProfitChartQuery query)
	{
		var dataSource = Scope
			.ServiceProvider
	...
	private static decimal RoundedSum(IEnumerable<SalesFact> facts, Func<SalesFact, decimal> selector) =>
		Math.Round(facts.Sum(selector), 12);

	private static DateTime StartOfDay(DateTime value) => new(
		value.Year,
		value.Month,
		value.Day,
		0,
	...
	private static DateTime EndOfDay(DateTime value) => StartOfDay(value).AddDays(1).AddTicks(-1);
}
