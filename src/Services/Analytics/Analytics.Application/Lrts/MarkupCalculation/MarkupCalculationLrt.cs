...
public class MarkupCalculationLrt(
    ...
    ILogger<MarkupCalculationLrt> logger) : LrtNamedObjectBase(jobRepository, unitOfWork, publisher, logger)
{
    ...
    protected override async Task DoWork()
    {
        ...
        var result = await analyzer.AnalyzeAsync(new MarkupAnalyzerInput
        {
        ...
        await UnitOfWork.ExecuteWithTransaction(
            ...
            async () =>
            {
                await Publisher.Publish(new MarkupAnalyzedEvent
                {
                    Ranges = ranges
                });
