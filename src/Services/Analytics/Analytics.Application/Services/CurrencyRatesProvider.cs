...
public class CurrencyRatesProvider(ICurrencyCacheRepository currencyCacheRepository) : ICurrencyRatesProvider
{
	...
	public Task<decimal?> GetRateOrDefault(int currencyId, CancellationToken cancellationToken = default) =>
		currencyCacheRepository.GetCurrencyRate(currencyId, cancellationToken);
}
