...
public class SaleFactSynchronizer(
	...
	ILogger<ISaleFactSynchronizer> logger) : ISaleFactSynchronizer
{
		...
		CancellationToken cancellationToken)
	{
		...
		var dbFact = await repository.GetFullSalesFact(saleDeletedEvent.SaleId, cancellationToken);
	...
	{
		var details = content.Details.Select(detail => SaleContentDetail.Create(
			detail.Id,
			content.Id,
			detail.CurrencyId,
			detail.BuyPrice,
