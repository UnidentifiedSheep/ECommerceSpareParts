	...
	: ChartDataSourceNamedObject<SalesProfitDataPoint, SalesProfitChartQuery>(schemaGenerator)
{
		...
		CancellationToken cancellationToken)
	{
		...
		return new ChartDataResult<SalesProfitDataPoint>(dataPoints, dataPoints.GetNextCursor(cursor));
	}
		...
		TimeGranularity granularity)
	{
		return query
			...
			{
				x.CreatedAt.Year,
				Month = granularity == TimeGranularity.Year ? 1 : x.CreatedAt.Month,
				Day = granularity == TimeGranularity.Day ? x.CreatedAt.Day : 1
			})
			.Select(group => new SalesProfitDataPoint
			{
				PeriodStart = new DateTime(
					...
					0,
					DateTimeKind.Utc),
				Revenue = Math.Round(group.Sum(x => x.RevenueInBaseCurrency), 12),
				Cost = Math.Round(group.Sum(x => x.CostInBaseCurrency), 12),
				GrossProfit = Math.Round(group.Sum(x => x.GrossProfitInBaseCurrency), 12),
