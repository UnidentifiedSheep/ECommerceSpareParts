...
internal static class CatalogueSearchQueryBuilder
{
		...
		Field producerIdField) where TDocument : class
	{
		...
		{
			boolean = boolean.Should(should.ToArray()).MinimumShouldMatch(1);

			return filters.Count == 0 ? boolean : boolean.Filter(filters);
		});
	}
	...
	private static List<Func<QueryContainerDescriptor<TDocument>, QueryContainer>> BuildSkuQueries<TDocument>(
		...
		Field field) where TDocument : class
	{
		...
		if (modes.Contains(SearchMatchType.Exact))
			queries.Add(descriptor => descriptor.Term(term => term.Field(field).Value(query).Boost(100)));
	...
	private static List<Func<QueryContainerDescriptor<TDocument>, QueryContainer>>
		BuildNameQueries<TDocument>(
			string query,
			IReadOnlySet<SearchMatchType> modes,
			Field field) where TDocument : class
	{
