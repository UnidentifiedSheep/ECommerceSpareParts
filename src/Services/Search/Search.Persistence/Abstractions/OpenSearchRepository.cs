...
public abstract class OpenSearchRepository<TDocument, TKey>(
	...
	Func<TDocument, TKey> idSelector) : ISearchRepository<TDocument, TKey>
	where TDocument : class where TKey : notnull
{
	protected IOpenSearchClient Client => client;

	public async Task<TDocument?> GetById(TKey id, CancellationToken cancellationToken = default)
	{
		...
		EnsureResponseSucceeded(response, $"get '{id}' from");
		return null;
	}

	public async Task Upsert(TDocument document, CancellationToken cancellationToken = default)
	{
		var index = await GetIndex(cancellationToken);
		var id = idSelector(document);
		var response = await client.IndexAsync(
			document,
			descriptor => descriptor.Index(index).Id(id.ToString()),
			cancellationToken);

		EnsureResponseSucceeded(response, $"upsert '{id}' into");
	}
	...
	{
		var distinctDocuments = documents.DistinctBy(idSelector).ToArray();
		if (distinctDocuments.Length == 0)
			return;
		...
		var response = await client.BulkAsync(
			descriptor => descriptor
				...
				.IndexMany(
					distinctDocuments,
					(operation, document) => operation.Id(idSelector(document).ToString())),
			cancellationToken);
	...
	public async Task Delete(TKey id, CancellationToken cancellationToken = default)
	{
		...
		EnsureResponseSucceeded(response, $"delete '{id}' from");
	}
		...
		var response = await client.BulkAsync(
			descriptor =>
			{
				...
				foreach (var id in distinctIds)
					descriptor.Delete<TDocument>(operation => operation.Id(id.ToString()));
	...
	protected async Task<string> GetIndex(CancellationToken cancellationToken = default)
	{
		await indexInitializer.LazyInitialize(cancellationToken);
		return indexProvider();
	}

	private void EnsureBulkSucceeded(BulkResponse response, string operation)
	{
		...
		var itemErrors = string.Join(
			...
			response.ItemsWithErrors.Select(item =>
				$"{item.Id}: {item.Error?.Reason ?? item.Status.ToString(CultureInfo.InvariantCulture)}"));
		var details = string.IsNullOrWhiteSpace(itemErrors) ? GetErrorDetails(response) : itemErrors;

		throw new InvalidOperationException(
			$"Failed to {operation} OpenSearch index '{indexProvider()}'. {details}");
	}

	protected void EnsureResponseSucceeded(IResponse response, string operation)
	{
		if (response.IsValid)
			return;
