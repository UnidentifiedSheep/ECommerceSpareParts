...
public class MainProducerSearchDocumentProvider(IMainClient mainClient) : IProducerSearchDocumentProvider
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		if (!response.Success)
			throw new InvalidOperationException(
				$"Unable to get producers from Main service. " + $"Status: {response.StatusCode}. " +
				$"Error: {response.Error}");

		var dict = response.ValueOrThrow.ToDictionary(x => x.Id);

		return idsList.ToDictionary(
			x => x,
			x => dict.TryGetValue(x, out var producer) ? MapProducer(producer) : null);
	}
