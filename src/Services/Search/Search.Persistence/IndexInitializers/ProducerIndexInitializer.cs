...
public class ProducerIndexInitializer(IOpenSearchClient client, IOptions<OpenSearchOptions> options)
			...
			ct => Client.Indices.CreateAsync(
				...
				c => c
					.Settings(s => s
						...
						.Analysis(a => a
							.Tokenizers(t => t.NGram(
								"producer_ngram_tokenizer",
								nt =>
									nt.MinGram(2).MaxGram(20).TokenChars(TokenChar.Letter, TokenChar.Digit)))
							.Analyzers(an => an.Custom(
								"producer_ngram_analyzer",
								ca => ca.Tokenizer("producer_ngram_tokenizer").Filters("lowercase")))
							.Normalizers(n => n.Custom(
								"lowercase_normalizer",
								cn => cn.Filters("lowercase")))))
					.Map<Producer>(m => m
						.Dynamic(false)
						.Properties(p => p
							...
							.Text(t => t
								...
								.Fields(f => f.Keyword(k =>
									k.Name("keyword").IgnoreAbove(256).Normalizer("lowercase_normalizer"))))
							.Text(t => t.Name(x => x.Description).Index(false))
							.Object<ProducerAlias>(o => o
								.Name(x => x.Aliases)
								.Dynamic(false)
								.Properties(op => op.Text(t =>
									t
										.Name(x => x.Alias)
										.Analyzer("producer_ngram_analyzer")
										.SearchAnalyzer("standard")
