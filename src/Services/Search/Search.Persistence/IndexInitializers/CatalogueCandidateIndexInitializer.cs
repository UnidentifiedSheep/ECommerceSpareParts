			...
			ct => Client.Indices.CreateAsync(
				...
				descriptor => descriptor
					.Settings(settings => settings
						...
						.Analysis(analysis => analysis.ConfigureCatalogueSearch()))
					.Map<CatalogueCandidate>(mapping => mapping
						.Dynamic(false)
						.Properties(properties => properties
							.Keyword(keyword => keyword.Name(x => x.Id))
							.Keyword(keyword => keyword.Name(x => x.Sku).Normalizer("lowercase_normalizer"))
							.Keyword(keyword => keyword
								...
								.Fields(fields => fields
									...
									.Text(contains => contains
										...
										.SearchAnalyzer(CatalogueSearchAnalysis.SearchAnalyzer))))
							.Number(number => number.Name(x => x.ProducerId).Type(NumberType.Integer))
							.Number(number => number.Name(x => x.MappedProductId).Type(NumberType.Integer))
							.Text(text => text
								.Name(x => x.Names)
