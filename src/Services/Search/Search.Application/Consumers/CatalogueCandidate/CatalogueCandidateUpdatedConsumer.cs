...
public sealed class CatalogueCandidateUpdatedConsumer(
    ISender sender)
    : IConsumer<Batch<CatalogueCandidateUpdatedEvent>>
{
    public Task Consume(
        ConsumeContext<Batch<CatalogueCandidateUpdatedEvent>> context)
    {
        return sender.Send(
            new UpsertCatalogueCandidatesCommand(
                context.Message
                    .Select(x => x.Message)
                    .ToList()),
...
public sealed class CatalogueCandidateUpdatedConsumerDefinition
    : ConsumerDefinition<CatalogueCandidateUpdatedConsumer>
{
