...
{
	[GraphQLName("search")]
	[RequireAllPermissions(PermissionCodes.ARTICLES_GET_MAIN, PermissionCodes.CATALOGUE_CANDIDATES_REVIEW)]
		...
		return new GqlCatalogueSearchResult
		{
			Products = new GqlSearchCatalogueSection<GqlProduct>
			{
				...
				Items = result
					...
					{
						Item = new GqlProduct(x.Id), Highlights = GqlHighlights.From(x.Highlights)
					})
					.ToArray()
			},
			Candidates = new GqlSearchCatalogueSection<GqlCatalogueCandidate>
			{
				...
				Items = result
					...
					{
						Item = new GqlCatalogueCandidate(x.Id), Highlights = GqlHighlights.From(x.Highlights)
					})
					.ToArray()
			}
		};
