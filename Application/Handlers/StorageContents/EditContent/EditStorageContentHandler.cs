...
public class EditStorageContentHandler(IStorageContentRepository storageContentRepository, IUnitOfWork unitOfWork,
    IConcurrencyValidator<StorageContent> concurrencyValidator, ICurrencyConverter currencyConverter,
    IMediator mediator, IArticlesService articlesService, IStoragesRepository storagesRepository,
    ICurrencyRepository currencyRepository, IUsersRepository usersRepository) : ICommandHandler<EditStorageContentCommand>
{
    public async Task<Unit> Handle(EditStorageContentCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var item in editedFields)
        {
            ...
            if (!concurrencyValidator.IsValid(content, clientConcurrencyCode, out string validCode))
    ...
    private (int diff, StorageMovement? movement) CalculateDiffAndMovement(StorageContent content, PatchStorageContentDto patch, string userId)
    {
    ...
    private async Task<Dictionary<int, StorageContent>> GetAndValidateStorageContents(IEnumerable<int> storageContentIds, CancellationToken cancellationToken)
    {
    ...
    private async Task ValidateData(IEnumerable<PatchStorageContentDto> values, string userId, CancellationToken cancellationToken = default)
    {
