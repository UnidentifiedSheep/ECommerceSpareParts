...
namespace Application.Handlers.Producers.AddOtherName;

[Transactional]
public record AddOtherNameCommand(int ProducerId, string OtherName, string? WhereUsed) : ICommand<Unit>;

public class AddOtherNameHandler(IProducerRepository producerRepository, IUnitOfWork unitOfWork) : ICommandHandler<AddOtherNameCommand>
{
    ...
    private async Task ValidateData(int producerId, string otherName, string? whereUsed, CancellationToken cancellationToken = default)
    {
