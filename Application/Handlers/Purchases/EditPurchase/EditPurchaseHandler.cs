...
public record EditPurchaseCommand(IEnumerable<EditPurchaseDto> Content, string PurchaseId, int CurrencyId, 
    string? Comment, string UpdatedUserId, DateTime PurchaseDateTime) : ICommand<EditPurchaseResult>;
...
public class EditPurchaseHandler(IUsersRepository usersRepository, ICurrencyRepository currencyRepository,
    IPurchaseRepository purchaseRepository, IUnitOfWork unitOfWork) : ICommandHandler<EditPurchaseCommand, EditPurchaseResult>
{
    public async Task<EditPurchaseResult> Handle(EditPurchaseCommand request, CancellationToken cancellationToken)
    {
        ...
        List<PurchaseContent> contentToAdd = new List<PurchaseContent>();
        ...
        foreach (var item in content)
        {
            ...
            { 
                ...
                int delta = item.Count - existingContent.Count;
    ...
    private void SetFields(Purchase purchase, int currencyId, DateTime purchaseDateTime, string? comment, string updatedUserId)
    {
    ...
    {
        await usersRepository.EnsureUsersExists([updatedUserId], ct: cancellationToken);
