...
public record GetPurchasesQuery(DateTime RangeStartDate, DateTime RangeEndDate, PaginationModel Pagination, string? SupplierId, 
    int? CurrencyId, string? SortBy, string? SearchTerm) : IQuery<GetPurchasesResult>;
public record GetPurchasesResult(IEnumerable<PurchaseDto> Purchases);



public class GetPurchasesHandler(IPurchaseRepository purchaseRepository) : IQueryHandler<GetPurchasesQuery, GetPurchasesResult>
{
    public async Task<GetPurchasesResult> Handle(GetPurchasesQuery request, CancellationToken cancellationToken)
    {
        ...
        var result = await purchaseRepository.GetPurchases(request.RangeStartDate, request.RangeEndDate, page, size, request.SupplierId,
