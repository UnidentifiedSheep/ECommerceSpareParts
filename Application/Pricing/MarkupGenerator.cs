    ...
    IMediator mediator) : IMarkupGenerator
{
    ...
    public async Task ReCalculateMarkupAsync(CancellationToken cancellationToken = default)
    {
        ...
        await mediator.Publish(new MarkupRangesUpdatedEvent(), cancellationToken);
    }

    private async Task ConvertBuySellToDefaultCurrencyAsync(CancellationToken cancellationToken)
    {
        var defaultCurrency = await currencyRepository.GetCurrencyById(_defaultSetting.DefaultCurrency, false, cancellationToken);
        ...
        {
            price.BuyPrice = Math.Round(currencyConverter.ConvertTo(price.BuyPrice, price.CurrencyId, defaultCurrency.Id), 2);
            price.SellPrice = Math.Round(currencyConverter.ConvertTo(price.SellPrice, price.CurrencyId, defaultCurrency.Id), 2);
    ...
    private async Task MarkOutliersAsync(CancellationToken cancellationToken)
    {
        ...
        for (decimal i = 0.11m; i <= maxBuyPrice;)
        {
    ...
    private async Task UpdateMarkUpRangesAsync(CancellationToken cancellationToken)
    {
        ...
        for (decimal i = 0.11m; i <= maxBuyPrice;)
        {
            ...
            var bucket = await buySellPriceRepository
                .GetBuySellPricesAsList(x => x.BuyPrice >= i && x.BuyPrice <= next && !x.IsOutLiner, false, cancellationToken);
            if (bucket.Count == 0)
            {
                stopCount++;
                i = next + 0.01m;
