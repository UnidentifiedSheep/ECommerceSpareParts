...
public class IntervalMapInt<T> : IntervalMap<Interval<T>> where T : class 
{
    private readonly int[] _map;
    public sealed override double MaxValue { get; protected set; }
    public IntervalMapInt(double maxValue)
    {
        MaxValue = maxValue;
        int scaledMaxValue = (int)(maxValue * ScaleFactor);
        _map = new int[scaledMaxValue + 1];
        Array.Fill(_map, -1); 
    ...
    public override IntervalMap<Interval<T>> AddInterval(Interval<T> interval)
    {
        ...
        if (Intervals.Count >= int.MaxValue)
            throw new InvalidOperationException("Количество интервалов превышает допустимые значения для int32");
    ...
    {
        int scaledValue = Scale(value);
        if (scaledValue < 0 || scaledValue >= _map.Length || _map[scaledValue] == -1 || !CheckAfterComa(value))
            return null;
