...
public interface IArticleReservationRepository
{
    ...
    Task<Dictionary<int, int>> GetReservationsCountForUserAsync(string userId, IEnumerable<int> articleIds,
        CancellationToken cancellationToken = default);

    Task<Dictionary<int, int>> GetReservationsCountForOthersAsync(string userId, IEnumerable<int> articleIds,
        CancellationToken cancellationToken = default);

    Task<Dictionary<int, List<StorageContentReservation>>> GetUserReservations(
        string userId, IEnumerable<int> articleIds, bool isDone = false, bool track = true,
        CancellationToken cancellationToken = default);
