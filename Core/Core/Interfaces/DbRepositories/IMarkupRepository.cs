...
public interface IMarkupRepository
{
    ...
    Task<IEnumerable<MarkupGroup>> GetMarkupGroups(int page, int viewCound, bool track = true, CancellationToken cancellationToken = default);
    Task<IEnumerable<MarkupRange>> GetMarkupRanges(int markupId, bool track = true, CancellationToken cancellationToken = default);
