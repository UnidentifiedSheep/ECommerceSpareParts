...
namespace Core.Services.Armtek;
...
public class SearchApi
{
    ...
    public SearchApi(IHttpClientFactory clientFactory, IConfiguration config)
    {
    ...
    public async Task<List<ArticleModel>> AssortementSearch(string vkorg, string articleNumber, CancellationToken cancellationToken = default)
    {
        if(vkorg.Length > 4 || articleNumber.Length > 39) throw new ArgumentException("vkorg length must be less than 4 and articleNumber must be shorter then 40");
        ...
        if (!response.IsSuccessStatusCode) throw new HttpRequestException($"Unable to get assortment: {responseString}");
