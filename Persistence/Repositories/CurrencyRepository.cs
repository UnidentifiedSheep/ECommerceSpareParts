...
public class CurrencyRepository(DContext context) : ICurrencyRepository
{
    ...
    public async Task<Currency?> GetCurrencyById(int id, bool track = true, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<bool> CurrencyExists(int id, CancellationToken cancellationToken = default)
        => await context.Currencies.AsNoTracking().AnyAsync(x => x.Id == id, cancellationToken);

    public async Task<IEnumerable<int>> CurrenciesExists(IEnumerable<int> ids, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IEnumerable<Currency>> GetCurrencies(IEnumerable<int> exceptIds, bool track = true, CancellationToken cancellationToken = default)
    {
        return await context.Currencies.ConfigureTracking(track)
    ...
    public async Task<bool> IsCurrencyCodeTaken(string code, CancellationToken cancellationToken = default)
        => await context.Currencies.AsNoTracking().AnyAsync(x => x.Code == code, cancellationToken);

    public async Task<bool> IsCurrencyShortNameTaken(string shortName, CancellationToken cancellationToken = default)
        => await context.Currencies.AsNoTracking().AnyAsync(x => x.ShortName == shortName, cancellationToken);

    public async Task<bool> IsCurrencyNameTaken(string name, CancellationToken cancellationToken = default)
        => await context.Currencies.AsNoTracking().AnyAsync(x => x.Name == name, cancellationToken);

    public async Task<bool> IsCurrencySignTaken(string sign, CancellationToken cancellationToken = default)
        => await context.Currencies.AsNoTracking().AnyAsync(x => x.CurrencySign == sign, cancellationToken);
}