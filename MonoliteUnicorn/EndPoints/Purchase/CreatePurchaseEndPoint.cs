...
public record CreatePurchaseRequest(string SupplierId, int CurrencyId, string StorageName, DateTime PurchaseDate,
        ...
        app.MapPost("/purchases/", async (HttpContext context, ISender sender, CreatePurchaseRequest request, CancellationToken token) =>
            {
                ...
                var command = new CreateFullPurchaseCommand(userId, request.SupplierId, request.CurrencyId, 
                    request.StorageName, request.PurchaseDate, request.PurchaseContent, request.Comment, request.PayedSum);
