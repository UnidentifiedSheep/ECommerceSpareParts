...
public class GetArticleCrossesEndPoint : ICarterModule
{
    ...
    {
        app.MapGet("/articles/crosses/{articleId}", async (ISender sender, ClaimsPrincipal user, int articleId, int viewCount, int page, string? sortBy, CancellationToken token) =>
            {
    ...
    private async Task<IResult> GetAmw(ISender sender, int articleId, PaginationModel pagination, string? sortBy, string userId, CancellationToken token)
    {
    ...
    private async Task<IResult> GetMember(ISender sender, int articleId, PaginationModel pagination, string? sortBy, string userId, CancellationToken token)
    {
