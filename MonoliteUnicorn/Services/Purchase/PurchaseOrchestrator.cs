...
public class PurchaseOrchestrator(IServiceProvider serviceProvider) : IPurchaseOrchestrator
{
        ...
        IEnumerable<NewPurchaseContentDto> purchaseContent, string? comment, decimal? payedSum, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await pricesService.RecalculateUsablePrice(inventoryItems.Select(x => (x.ArticleId, x.BuyPriceInUsd)), cancellationToken);
        ...
        string updatedUserId, DateTime purchaseDateTime, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var purchase = await context.Purchases.AsNoTracking()
                ...
                .FirstOrDefaultAsync(x => x.Id == purchaseId, cancellationToken) ?? throw new PurchaseNotFoundException(purchaseId);
            var newTotalSum = contentList.Sum(x => x.Count * x.Price);
            var editArticleCounts = await purchaseService.EditPurchaseAsync(contentList, purchaseId, currencyId, comment, updatedUserId, purchaseDateTime, cancellationToken);
            await balanceService.EditTransaction(purchase.TransactionId, currencyId, newTotalSum, TransactionStatus.Purchase, purchaseDateTime, cancellationToken);
            await inventoryService.AddOrRemoveContentFromStorage(editArticleCounts, purchase.CurrencyId, purchase.Storage, 
                purchase.PurchaseDatetime, purchaseDateTime, updatedUserId, StorageMovementType.PurchaseEditing, cancellationToken);
            await dbTransaction.CommitAsync(cancellationToken);
        }
        catch (Exception)
        {
    ...
    public async Task DeletePurchase(string purchaseId, string userId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await inventoryService.AddOrRemoveContentFromStorage(content, purchase.CurrencyId, purchase.Storage, 
                purchase.PurchaseDatetime, purchase.PurchaseDatetime, userId, StorageMovementType.PurchaseDeletion, cancellationToken);
